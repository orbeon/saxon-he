...
public class ForEachGroup extends Instruction
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        ...
        keyItemType = key.getItemType().getPrimitiveType();
        for (Operand o : operands()) {
    ...
    private static void fixupGroupReferences(Expression exp, ForEachGroup feg, ItemType selectedItemType, boolean isInLoop) {
        if (exp == null) {
            // no action
        } else if (exp instanceof CurrentGroup) {
            ...
            /// TODO: can also occur in group-by, group-adjacent, etc??
            ...
            for (Operand o : exp.operands()) {
                fixupGroupReferences(o.getExpression(), feg, selectedItemType, isInLoop || o.isEvaluatedRepeatedly());
            }
        }
    }
    ...
    public Expression optimize(ExpressionVisitor visitor, ContextItemStaticInfo contextItemType) throws XPathException {
        ...
        ItemType selectedItemType = select.getItemType();
        ContextItemStaticInfo sit = new ContextItemStaticInfo(selectedItemType, false, select);
        ...
        if (collator == null && (collationNameExpression instanceof StringLiteral)) {
            ...
            try {
                ...
                if (!collationURI.isAbsolute()) {
                    ...
                    collationNameExpression = new StringLiteral(collationNameString, getContainer());
    ...
    public Iterable<Operand> operands() {
        ...
        list.add(new Operand(action, OperandRole.FOCUS_CONTROLLED_ACTION));
        list.add(new Operand(key, OperandRole.NEW_FOCUS_ATOMIC));
        if (collationNameExpression != null && !(collationNameExpression instanceof Literal)) {
            list.add(new Operand(collationNameExpression, OperandRole.SINGLE_ATOMIC));
        }
        if (sortKeyDefinitions != null) {
            for (SortKeyDefinition skd : sortKeyDefinitions) {
...
//#ifdefined STREAM
    ...
    private static boolean containsNestedForEachGroup(Expression exp) {
        for (Operand o : exp.operands()) {
            Expression child = o.getExpression();
            if (child instanceof ForEachGroup || containsNestedForEachGroup(child)) {
                return true;
            }
        }
        return false;
    ...
    public TailCall processLeavingTail(XPathContext context) throws XPathException {
        ...
        FocusIterator focusIterator = new FocusTrackingIterator(groupIterator);
    ...
    private StringCollator getCollator(XPathContext context) throws XPathException {
        if (collationNameExpression != null) {
            ...
            try {
                return context.getConfiguration().getCollation(cname, baseURI.toString(), "FOCH0002");
    ...
    private GroupIterator getGroupIterator(XPathContext context) throws XPathException {
        FocusIterator population = new FocusTrackingIterator(select.iterate(context));
        ...
        switch (algorithm) {
            case GROUP_BY: {
                ...
                groupIterator = new GroupByIterator(population, key, c2, coll, composite);
                break;
            }
            case GROUP_ADJACENT: {
                StringCollator coll = collator;
                ...
                groupIterator = new GroupAdjacentIterator(population, key, context, coll, composite);
                break;
            }
            case GROUP_STARTING:
    ...
    public SequenceIterator iterate(XPathContext context) throws XPathException {
        ...
        FocusIterator focus = new FocusTrackingIterator(master);
        XPathContextMajor c2 = context.newContext();
        c2.setOrigin(this);
