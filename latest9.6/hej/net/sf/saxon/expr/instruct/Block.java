...
public class Block extends Instruction {
    ...
    public static Expression makeBlock(/*@Nullable*/ Expression e1, Expression e2) {
        ...
        if (e1 instanceof Block || e2 instanceof Block) {
            ...
            if (e1 instanceof Block) {
                Collections.addAll(list, ((Block) e1).children);
            } else {
                list.add(e1);
            }
            if (e2 instanceof Block) {
                Collections.addAll(list, ((Block) e2).children);
            } else {
                list.add(e2);
            }
     ...
     * @param container the container for the new Block
     * @return a Block containing the two subexpressions, and if either of them is a block, it will
     *         have been collapsed to create a flattened sequence
     */

    public static Expression makeBlock(List<Expression> list, Container container) {
        if (list.size() == 0) {
            return Literal.makeEmptySequence(container);
        } else if (list.size() == 1) {
            return list.get(0);
        } else {
            Expression[] exps = new Expression[list.size()];
        ...
        if (hasAdjacentTextNodes) {
            ...
            for (int i = 0; i < children.length; i++) {
                if (isLiteralText[i]) {
                    ...
                    if (pendingText != null) {
                        ValueOf inst = new ValueOf(new StringLiteral(pendingText, getContainer()), false, false);
                        content.add(inst);
                        pendingText = null;
                    }
            ...
            return makeBlock(content, getContainer());
        } else {
            return this;
        }
    }
    ...
    public Iterable<Operand> operands() {
        ...
        for (Expression child : children) {
            list.add(new Operand(child, OperandRole.SAME_FOCUS_ACTION));
        }
        return list;
    }
...
//    }    
    ...
    public final ItemType getItemType() {
        ...
        for (int i = 1; i < children.length; i++) {
            t1 = Type.getCommonSuperType(t1, children[i].getItemType(), th);
    ...
    public Expression simplify(ExpressionVisitor visitor) throws XPathException {
        ...
        if (children.length == 0) {
            Expression result = Literal.makeEmptySequence(getContainer());
        ...
        if (allAtomic) {
            ...
            Expression result = Literal.makeLiteral(new SequenceExtent(values), getContainer());
            ExpressionTool.copyLocationInfo(this, result);
            return result;
        }
    ...
    private void flushCurrentLiteralList(List<Item> currentLiteralList, List<Expression> list) throws XPathException {
        if (currentLiteralList != null) {
            ...
            list.add(Literal.makeLiteral(SequenceExtent.makeSequenceExtent(iter), getContainer()));
        }
    }
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        for (int c = 0; c < children.length; c++) {
            children[c] = visitor.typeCheck(children[c], contextInfo);
    ...
    public String toShortString() {
        return "(" + children[0].toShortString() + ", ...)";
    }
