...
public class Optimizer {
    ...
    public boolean isVariableReplaceableByDot(Expression exp, Binding[] binding) {
        ...
        if (exp instanceof ContextSwitchingExpression) {
            ...
            for (Operand o : exp.operands()) {
                if (!isVariableReplaceableByDot(o.getExpression(), binding)) {
                    return false;
                }
            }
            return true;
    ...
    public RuleTarget makeInversion(Pattern pattern, Template template) throws XPathException {
        return null;
    }
...
//#ifdefined HOF
    public FunctionItem makeCastToUnion(final UnionType type, final NamespaceResolver resolver) {
        // The only possible UnionType is ErrorType
        Callable callable = new Callable() {
            public Sequence call(XPathContext context, Sequence[] arguments) throws XPathException {
                throw new XPathException("Casting to xs:error always fails", "XPTY0004");
            }
        };
        return new CallableFunctionItem(1, callable, AnyFunctionType.getInstance());
    }
//#endif
    ...
    public void trace(String message, Expression exp) {
        if (tracing) {
            ...
            err.info("OPT ======================================");
            err.info("OPT : At line " + exp.getLineNumber() + " of " + exp.getSystemId());
