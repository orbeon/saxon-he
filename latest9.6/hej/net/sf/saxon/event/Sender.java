        ...
        if (parserFeatures != null) {
            for (Map.Entry<String, Boolean> entry : parserFeatures.entrySet()) {
                try {
                    ...
                    config.getLogger().warning("XML Parser does not recognize the feature " + entry.getKey());
                } catch (SAXNotSupportedException err) {
                    config.getLogger().warning("XML Parser does not support the feature " + entry.getKey());
                }
            }
        }
        ...
        if (parserProperties != null) {
            for (Map.Entry<String, Object> entry : parserProperties.entrySet()) {
                try {
                    ...
                    config.getLogger().warning("XML Parser does not recognize the property " + entry.getKey());
                } catch (SAXNotSupportedException err) {
                    config.getLogger().warning("XML Parser does not support the property " + entry.getKey());
                }
...
//#if EE==true
            ...
            if (!(receiver instanceof DocumentValidator)) {
...
//#endif
    ...
    private static void sendPullEventSource(PullEventSource source, Receiver receiver, ParseOptions options)
        ...
        receiver.setPipelineConfiguration(pipe);
        SequenceReceiver out = SequenceReceiver.isTrueSequenceReceiver(receiver)
