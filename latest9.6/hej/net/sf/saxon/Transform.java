...
public class Transform {
            ...
            if (useAssociatedStylesheet) {
                ...
                if (stylesheetSources.size() > 1) {
                    for (int j = 0; j < repeat; j++) {
                        ...
                        List<XsltPackage> xsltPackages = (List<XsltPackage>) compiler.compilePackages(stylesheetSources);
    ...
    private void processFile(/*@Nullable*/ Source source, XsltExecutable sheet, /*@Nullable*/ File outputFile, CommandLineOptions options,
                             Logger traceDestination)
            throws SaxonApiException {
        ...
        for (int r = 0; r < repeat; r++) {      // repeat is for internal testing/timing
            ...
            if (showTime) {
                ...
                getConfiguration().getNamePool().statistics();
                if (repeat > 1) {
                    System.err.println("-------------------------------");
    ...
    protected void badUsage(String message) {
        ...
        System.err.println("Usage: see http://www.saxonica.com/documentation/html/using-xsl/commandline.html");
