...
public class FormatInteger extends SystemFunctionCall implements Callable {
    ...
    public static final String preface = "In the picture string supplied to format-integer(), ";
    ...
    public StringValue evaluateItem(XPathContext context) throws XPathException {
        return formatInteger(
                (IntegerValue) argument[0].evaluateItem(context),
                (StringValue) argument[1].evaluateItem(context),
                argument.length == 2 ? null : (StringValue) argument[2].evaluateItem(context),
                context);
    }
    ...
    private StringValue formatInteger(IntegerValue num, StringValue picture, /*@Nullable*/ StringValue language, XPathContext context) throws XPathException {
        ...
        if (pic.length() == 0) {
            XPathException err = new XPathException(preface + "the picture cannot be empty", "FODF1310");
            err.setLocator(this);
            throw err;
        }
        ...
        if (lastSemicolon >= 0) {
            ...
            if (primaryToken.length() == 0) {
                XPathException err = new XPathException(preface + "the primary format token cannot be empty", "FODF1310");
                err.setLocator(this);
                throw err;
            }
            modifier = lastSemicolon < pic.length() - 1 ? pic.substring(lastSemicolon + 1) : "";
            if (!modifierPattern.matches(modifier)) {
                XPathException err = new XPathException(preface + "the modifier is invalid", "FODF1310");
                err.setLocator(this);
                throw err;
            }
        } else {
        ...
        if (isDecimalDigitPattern) {
            if (!decimalDigitPattern.matches(primaryToken)) {
                XPathException err = new XPathException(
                        preface + "the primary format token contains a decimal digit but does not meet the rules for a decimal digit pattern", "FODF1310");
                err.setLocator(this);
                throw err;
            }
     ...
     */
    private NumericGroupFormatter getPicSeparators(String pic) throws XPathException {
        ...
        if (badHashPattern.matches(pic)) {
            XPathException err = new XPathException(preface + "the picture is not valid (it uses '#' where disallowed)", this);
            ...
            switch (Character.getType(codePoint)) {
                ...
                case Character.DECIMAL_DIGIT_NUMBER:
                    ...
                    if (zeroDigit == -1) {
                        ...
                        if (zeroDigit != Alphanumeric.getDigitFamily(codePoint)) {
                            XPathException err = new XPathException(
                                    preface + "the picture mixes digits from different digit families");
                            err.setErrorCode("FODF1310");
                            err.setLocator(this);
                            throw err;
                        }
                ...
                default:
                    if (i == picExpanded.uLength() - 1) {
                        XPathException err = new XPathException(preface + "the picture cannot end with a separator", this);
                        err.setErrorCode("FODF1310");
                        throw err;
                    }
                    if (codePoint == '#') {
                        ...
                        if (i != 0) {
                            switch (Character.getType(picExpanded.uCharAt(i - 1))) {
                                ...
                                case Character.OTHER_LETTER:
                                    XPathException err = new XPathException(
                                            preface + "the picture cannot contain alphanumeric character(s) before character '#'", this);
                                    err.setErrorCode("FODF1310");
                                    throw err;
                            }
                        }
//                        if (picExpanded.charAt(i + 1) == singleSeparator) {
//                            picExpanded = picExpanded.substring(i + 3, picExpanded.length());
//                        } else {
//                            picExpanded = picExpanded.substring(i + 2, picExpanded.length());
//                        }
                        //i = 0;
                        break;
                    } else {
                        ...
                        if (!added) {
                            XPathException err = new XPathException(preface + "the picture contains consecutive separators", this);
                            err.setErrorCode("FODF1310");
                            err.setLocator(this);
                            throw err;
                        }
                        ...
                        if (i == 0) {
                            XPathException err = new XPathException(preface + "the picture cannot begin with a separator", this);
                            err.setErrorCode("FODF1310");
                            err.setLocator(this);
                            throw err;
                        }
                        //singleSeparator = codePoint;
                        //j++;
                    }
            }
        }
        ...
        if (groupingPositions.size() == 0) {
            //UnicodeString originalPic = UnicodeString.makeUnicodeString(pic);
            //UnicodeString adjustedPic = originalPic.substring(picLen - picExpanded.length(), originalPic.length());
     ...
     */
    private UnicodeString extractSeparators(UnicodeString arr, IntSet excludePositions) {
