...
public abstract class SystemFunctionCall extends FunctionCall implements Callable {
            ...
            public Sequence call(XPathContext context, Sequence[] arguments) throws XPathException {
                ...
                for (int i = 0; i < arguments.length; i++) {
                    ...
                    convertedArgs[i] = th.applyFunctionConversionRules(
                            arguments[i], requiredType, role, SystemFunctionCall.this.getContainer());
                }
    ...
    /*@Nullable*/
    public static FunctionCall makeSystemFunction(String name, /*@NotNull*/ Expression[] arguments) {
...
//#ifdefined HOF
    ...
    public FunctionItemType getFunctionItemType(TypeHierarchy th) {
        ...
        return new com.saxonica.functions.hof.SpecificFunctionType(argTypes,
...
//#endif
        ...
        if (sf == this && argument.length <= details.resultIfEmpty.length) {
            ...
            for (int i = 0; i < argument.length; i++) {
                if (Literal.isEmptySequence(argument[i]) && details.resultIfEmpty[i] != null) {
                    return Literal.makeLiteral(SequenceTool.toGroundedValue(details.resultIfEmpty[i]), getContainer());
                }
            }
        }
        return sf;
    ...
    public Iterable<Operand> operands() {
        try {
            List<Operand> list = new ArrayList<Operand>(argument.length);
            for (int i = 0; i < argument.length; i++) {
                list.add(new Operand(argument[i],
                        new OperandRole(0, details.usage[i], details.argumentTypes[i])));
            }
            return list;
    ...
    public ItemType getItemType() {
        ...
        if ((details.properties & StandardFunction.AS_ARG0) != 0) {
            ...
            if (argument.length > 0) {
                ItemType t0 = argument[0].getItemType().getPrimitiveItemType();
...
//        setDetails(StandardFunction.getFunction(augmentedName, newArgs.length));
    ...
    /*@Nullable*/
    public PathMap.PathMapNodeSet addDocToPathMap(/*@NotNull*/ PathMap pathMap, PathMap.PathMapNodeSet pathMapNodes) {
    ...
    public String toShortString() {
        return getDisplayName() + "(" + (getNumberOfArguments()==0 ? "" : "...") + ")";
    }
}
