...
//    }
    ...
    public net.sf.saxon.z.IntPredicate getValidCharacterChecker() {
        if (xmlVersion == XML10) {
            return new net.sf.saxon.z.IntPredicate() {
...
//        }
     ...
     * @throws net.sf.saxon.trans.XPathException if this version of Saxon does not support the requested options
     */

    public XPathParser newExpressionParser(String language, boolean updating, DecimalValue languageVersion) throws XPathException {
        if ("XQ".equals(language)) {
            if (updating) {
                throw new XPathException("XQuery Update is supported only in Saxon-EE");
            } else if (DecimalValue.THREE.equals(languageVersion) || DecimalValue.ONE.equals(languageVersion)) {
                // In Saxon-HE, ignore the request for a 3.0 parser
                XQueryParser parser = new XQueryParser();
                parser.setLanguage(XPathParser.XQUERY, languageVersion);
                return parser;
            } else {
                throw new XPathException("Unknown XQuery version " + languageVersion);
            }
        } else if ("XP".equals(language)) {
            if (DecimalValue.THREE.equals(languageVersion) || DecimalValue.TWO.equals(languageVersion)) {
    ...
    public Sequence makeClosure(Expression expression, int ref, XPathContext context) throws XPathException {
        return ref > 1 ? new MemoClosure() : new Closure();
    }
    ...
    /**
     * Get the global location map. Used only for XSLT 3.0 packages, therefore
     * not available in Saxon-HE. This version of the method always returns null.
     * @return the global location map, always null in Saxon-HE and Saxon-PE
     */

    public LocationMap getGlobalLocationMap() {
        return null;
    }
...
//    }
    ...
    public void setConfigurationProperty(String name, Object value) {
        ...
        if (booleanPropertyNames.contains(name)) {
            ...
            if (value instanceof Integer) {
                ...
                optimizationLevel = (Integer) value;
            } else {
            ...
            if (name.equals(FeatureKeys.XQUERY_ALLOW_UPDATE)) {
                ...
                XPathParser parser = new XPathParser();
                parser.setLanguage(XPathParser.SEQUENCE_TYPE, DecimalValue.THREE);
                try {
    ...
    public Expression rewriteCopyOnEmpty(XSLCopy copy, Copy inst, Expression select, Expression onEmpty) {
        throw new UnsupportedOperationException("xsl:copy/on-empty requires Saxon-EE");
    }
...
//#endif
    ...
    public static String getSoftwareEdition() {
