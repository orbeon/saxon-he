...
public abstract class StyleElement extends ElementImpl
    ...
    public PackageData getPackageData() {
        return compilation.getPackageData();
    }
    ...
    public int getDefaultValidation() {
        XSLModuleRoot stylesheet = getContainingStylesheet();
        if (stylesheet != null) {
     ...
     *          if any of the standard attributes is incorrect
     */
     ...
     * @throws net.sf.saxon.trans.XPathException
     *          if a static error is detected in the XPath expression
     */
     ...
     * @throws net.sf.saxon.trans.XPathException
     *          if a static error is detected in the pattern
     */

    public Pattern makePattern(String pattern)
            throws XPathException {
        try {
            return Pattern.make(pattern, getStaticContext(), getCompilation().getPackageData());
        } catch (XPathException err) {
            ...
            nsp.setPackageData(getCompilation().getPackageData());
            return nsp;
        }
    }
     ...
     * @throws net.sf.saxon.trans.XPathException
     *          if a static error is detected in the AVT
     */

    protected Expression makeAttributeValueTemplate(String expression)
            throws XPathException {
        try {
            return AttributeValueTemplate.make(expression, getLineNumber(), staticContext, this);
        } catch (XPathException err) {
            compileError(err);
            return new StringLiteral(expression, this);
        }
    }
     ...
     * @throws net.sf.saxon.trans.XPathException
     *          if the value given for the attribute is not a permitted value
     */
    ...
    public SequenceType makeSequenceType(String sequenceType)
        ...
        try {
            ...
            parser.setLanguage(XPathParser.XPATH, staticContext.getXPathLanguageLevel());
    ...
    protected void processVersionAttribute(String ns) throws XPathException {
        ...
        if (v != null) {
            ...
            if (val instanceof ValidationFailure) {
                ...
                if (version.compareTo(DecimalValue.TWO) < 0 && !version.equals(DecimalValue.ONE)) {
     ...
     *
     * @throws net.sf.saxon.trans.XPathException
     *          if the value is not a valid URI, or not a recognized collation URI
     */
        ...
        try {
            ...
            if (typeAtt.startsWith("Q{")) {
                try {
                    ...
                    nc = getNamePool().allocate("", uri, lname);
                ...
                nc = getNamePool().allocate(parts[0], uri, lname);
            }
    ...
    public Expression typeCheck(String name, Expression exp) throws XPathException {
        ...
        try {
            exp = makeExpressionVisitor().typeCheck(exp, new ContextItemStaticInfo(Type.ITEM_TYPE, true));
    ...
    public void allocatePatternSlots(int slots) {
        getContainingPackage().allocatePatternSlots(slots);
    }
    ...
    public Pattern typeCheck(String name, Pattern pattern) throws XPathException {
        ...
        try {
            pattern = pattern.analyze(makeExpressionVisitor(), new ContextItemStaticInfo(Type.NODE_TYPE, true));
            ...
            if (err.isReportableStatically()) {
                ...
                Pattern p = new PatternWithPredicate(
                        new NodeTestPattern(ErrorType.getInstance()), new ErrorExpression(err));
                p.setPackageData(compilation.getPackageData());
                return p;
            }
        }
    }
    ...
    protected void validateChildren(ComponentDeclaration decl, boolean excludeStylesheet) throws XPathException {
        ...
        while ((child = kids.next()) != null) {
            if (child instanceof StyleElement) {
                ...
                endsWithTextTemplate = getCompilation().getStyleNodeFactory(true).validateTextNode(child);
            }
        }
    ...
    public Expression compileSequenceConstructor(Compilation compilation, ComponentDeclaration decl,
        ...
        while ((node = (NodeInfo) iter.next()) != null) {
            ...
            if (node.getNodeKind() == Type.TEXT) {
                if (isExpandingText()) {
                    getCompilation().getStyleNodeFactory(true).compileContentValueTemplate((TextImpl) node, contents, this);
                } else {
                    ...
                    if (!(sibling instanceof XSLLocalParam || sibling instanceof XSLSort)) {
                        ...
                        Expression text = new ValueOf(new StringLiteral(node.getStringValue(), this), false, false);
    ...
    protected AttributeSet[] getAttributeSets(String use, /*@Nullable*/ List<ComponentDeclaration> list)
            throws XPathException {
    ...
    protected WithParam[] getWithParamInstructions(Compilation compilation, ComponentDeclaration decl, boolean tunnel)
            throws XPathException {
        int count = 0;
        AxisIterator kids = iterateAxis(AxisInfo.CHILD);
        NodeInfo child;
    ...
    protected void invalidAttribute(String attributeName, String allowedValues) throws XPathException {
        compileError("Attribute " + getDisplayName() + "/@" + attributeName + " must be " + allowedValues, "XTSE0020");
    }
    ...
    public SourceBinding bindVariable(StructuredQName qName) {
        ...
        SourceBinding binding = getContainingPackage().getGlobalVariableBinding(qName);
