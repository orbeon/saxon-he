...
public class XQueryParser extends XPathParser {
    ...
    public XQueryExpression makeXQueryExpression(
        ...
        try {
            ...
            if (streaming) {
                env.getConfiguration().checkLicensedFeature(Configuration.LicenseFeature.ENTERPRISE_XQUERY, "streaming");
            }
...
//            }
    ...
    private Expression parseQuery(String queryString,
        ...
        t.languageLevel = allowXPath30Syntax ? DecimalValue.THREE : DecimalValue.TWO;
        parseProlog();
        processPreamble();
        ...
        while (true) {
            ...
            if (uri.equals(NamespaceConstant.XQUERY)) {
                if (qName.equals(Annotation.PRIVATE) || qName.equals(Annotation.PUBLIC) ||
                    ...
                    grumble("Unrecognized variable or function annotation " + qName.getDisplayName());
                }
            ...
            if (t.currentToken == Token.LPAR) {
                ...
                while (true) {
                    ...
                    if (arg instanceof Literal) {
                        GroundedValue val = ((Literal) arg).getValue();
...
//        }
    ...
    private void parseDecimalFormatProperties(/*@Nullable*/ StructuredQName formatName) throws XPathException {
        ...
        dfs.setHostLanguage(Configuration.XQUERY);
    ...
    private void parseContextItemDeclaration() throws XPathException {
        ...
        if (exec.getInitialContextItemVariable() != null) {
            // the context item is already declared in another module. Compare the required types
            GlobalVariable otherVar = exec.getInitialContextItemVariable();
            ItemType otherType = otherVar.getRequiredType().getPrimaryType();
            if (otherType != AnyItemType.getInstance()) {
                TypeHierarchy th = env.getConfiguration().getTypeHierarchy();
                int rel = th.relationship(requiredType.getPrimaryType(), otherType);
                if (rel == TypeHierarchy.SAME_TYPE || rel == TypeHierarchy.SUBSUMES) {
                    ...
                    if (var.getSelectExpression() != null && otherVar.getSelectExpression() == null) {
                        otherVar.setSelectExpression(var.getSelectExpression());
                    }
                    ...
                    if (var.getSelectExpression() == null && otherVar.getSelectExpression() != null) {
                        var.setSelectExpression(otherVar.getSelectExpression());
                    }
                    ...
                    if (!(requiredType.getPrimaryType() instanceof NodeTest && otherType instanceof NodeTest)) {
                        throw new AssertionError("Overlapping types found that are not NodeTests");
                    }
                    ItemType intersection = new CombinedNodeTest(
                            ((NodeTest) requiredType.getPrimaryType()),
                            Token.INTERSECT,
                            (NodeTest) otherType);
                    otherVar.setRequiredType(
                            SequenceType.makeSequenceType(intersection, StaticProperty.EXACTLY_ONE));
                    if (var.getSelectExpression() != null && otherVar.getSelectExpression() == null) {
                        otherVar.setSelectExpression(var.getSelectExpression());
                    }
                }
    ...
    protected void parseOutputDeclaration(StructuredQName varName, String value) throws XPathException {
        ...
        if (localName.equals("parameter-document")) {
            ...
            Source source;
            try {
                source = env.getConfiguration().getURIResolver().resolve(href, env.getBaseURI());
            } catch (TransformerException e) {
                throw XPathException.makeXPathException(e);
            }
            ParseOptions options = new ParseOptions();
            options.setSchemaValidationMode(Validation.LAX);
            DocumentInfo doc = env.getConfiguration().buildDocument(source);
            SerializationParamsHandler ph = new SerializationParamsHandler();
            ph.setSerializationParams(doc);
        ...
        while (true) {
            ...
            if (t.currentToken == Token.FOR) {
                ...
                for (int i = 0; i < keys.length; i++) {
                    ...
                    SortKeyDefinition key = new SortKeyDefinition(defaultContainer);
                    key.setSortKey(((SortSpec) sortSpecList.get(i)).sortKey, false);
                    key.setOrder(new StringLiteral(spec.ascending ? "ascending" : "descending", defaultContainer));
    ...
    private void checkForClauseAllowingEmpty(net.sf.saxon.expr.flwor.ForClause clause) throws XPathException {
        if (!allowXPath30Syntax) {
        ...
        if (!Cardinality.allowsZero(type.getCardinality())) {
            ...
            Expression[] args = {clause.getSequence()};
            clause.setSequence(SystemFunctionCall.makeSystemFunction("one-or-more", args));
            // TODO: wrapping one-or-more() has the desired effect of checking cardinality, but produces
            // poor error messages (test outer-013.xq). But other approaches to dynamic checking, such as
            // adding a cardinality checker, don't work because it's too early to call them during parsing.
        }
    }
     ...
     *
     */
    private void parseGroupByClause(List<Clause> clauseList) throws XPathException {
        if (!allowXPath30Syntax) {
        ...
        clause.setGroupingTupleExpression(groupingTupleExpr);
        ...
        clause.setRetainedTupleExpression(retainedTupleExpr);
     ...
     *
     */
    private void parseWindowClause(List<Clause> clauseList) throws XPathException {
        if (!allowXPath30Syntax) {
        ...
        t.setState(Tokenizer.DEFAULT_STATE);
        nextToken();
        clause.setStartCondition(parseExprSingle());
        if (isKeyword("only")) {
        ...
        if (isKeyword("end")) {
            ...
            t.setState(Tokenizer.DEFAULT_STATE);
            nextToken();
            clause.setEndCondition(parseExprSingle());
        } else {
    ...
    public static Expression makeStringJoin(Expression exp, /*@NotNull*/ StaticContext env) {
        ...
        if (exp.getCardinality() == StaticProperty.EXACTLY_ONE) {
            ...
            StringJoin fn = (StringJoin) SystemFunctionCall.makeSystemFunction(
                    "string-join", new Expression[]{exp, new StringLiteral(StringValue.SINGLE_SPACE, exp.getContainer())});
...
//        }
    ...
    protected Expression parseSwitchExpression() throws XPathException {
        ...
        conditions.add(Literal.makeLiteral(BooleanValue.TRUE, defaultContainer));
        ...
        return makeTracer(offset, outerLet, Location.SWITCH_EXPRESSION, null);
    }
...
//            }
    ...
    private Expression parseComputedElementConstructor(int offset) throws XPathException {
        ...
        if (name instanceof Literal) {
            ...
            if (content == null) {
                content = Literal.makeEmptySequence(defaultContainer);
            }
            if (!((QueryModule) env).isPreserveNamespaces()) {
                content = new CopyOf(content, false, Validation.PRESERVE, null, true);
            }
    ...
    private Expression parseNamedAttributeConstructor(int offset) throws XPathException {
        ...
        if (warning == null) {
            ...
            return makeTracer(offset, att2, Location.LITERAL_RESULT_ATTRIBUTE, attributeName.getStructuredQName());
        } else {
            warning(warning);
    ...
    private Expression parseNamedProcessingInstructionConstructor(int offset) throws XPathException {
        ...
        Expression piName = new StringLiteral(target, defaultContainer);
    ...
    private Expression parseNamedNamespaceConstructor(int offset) throws XPathException {
        ...
        Expression nsName = new StringLiteral(target, defaultContainer);
    ...
    protected void makeSimpleContent(/*@Nullable*/ Expression content, /*@NotNull*/ SimpleNodeConstructor inst, int offset) throws XPathException {
        try {
            if (content == null) {
                inst.setSelect(new StringLiteral(StringValue.EMPTY_STRING, defaultContainer), env.getConfiguration());
            } else {
            ...
            switch (c) {
                ...
                case '/':
                    if (allowEndTag) {
                        ...
                        return new StringLiteral(sb.toString(), defaultContainer);
                    }
...
//            }
    ...
    private Expression makeAttributeContent(/*@NotNull*/ String avt,
        ...
        while (last < len) {
            ...
            if ((i0 < 0 || i2 < i0) && (i8 < 0 || i2 < i8)) {   // found end of string
                ...
                if (i2 + 1 < avt.length() && avt.charAt(i2 + 1) == terminator) {
                    components.add(new StringLiteral(terminator + "", defaultContainer));
                    last = i2 + 2;
                    //continue;
                } else {
                    last = i2;
        ...
        if (scanOnly) {
            return Literal.makeLiteral(Int64Value.makeIntegerValue(last), defaultContainer);
        }
    ...
    private void addStringComponent(/*@NotNull*/ List<Expression> components, /*@NotNull*/ String avt, int start, int end)
        ...
        if (start < end) {
            ...
            components.add(new StringLiteral(sb.toString(), defaultContainer));
        }
    }
        ...
        try {
            ...
            while (true) {
                ...
                if (text.length() > 0 &&
                    ...
                    ValueOf inst = new ValueOf(new StringLiteral(new StringValue(text.condense()), defaultContainer), false, false);
                    setLocation(inst);
                    components.add(inst);
                ...
                if (c == '<') {
                    ...
                    if (afterEnclosedExpr) {
                        ...
                        if (!(previousItemType instanceof NodeTest)) {
                            ...
                            ValueOf inst = new ValueOf(new StringLiteral(StringValue.EMPTY_STRING, defaultContainer), false, false);
                            setLocation(inst);
                            components.add(inst);
                        }
                    }
    ...
    private Expression parsePIConstructor() throws XPathException {
        try {
            ...
            ProcessingInstruction instruction =
                    new ProcessingInstruction(new StringLiteral(target, defaultContainer));
            instruction.setSelect(new StringLiteral(data, defaultContainer), env.getConfiguration());
    ...
    private Expression parseCommentConstructor() throws XPathException {
        try {
            ...
            instruction.setSelect(new StringLiteral(new StringValue(commentText), defaultContainer), env.getConfiguration());
            setLocation(instruction);
            return instruction;
        } catch (StringIndexOutOfBoundsException err) {
    ...
    public static Expression stringify(Expression exp, boolean noNodeIfEmpty, Configuration config) throws XPathException {
        ...
        exp = SystemFunctionCall.makeSystemFunction("string-join",
                new Expression[]{exp, new StringLiteral(StringValue.SINGLE_SPACE, exp.getContainer())});
        assert exp != null;
        if (noNodeIfEmpty) {
