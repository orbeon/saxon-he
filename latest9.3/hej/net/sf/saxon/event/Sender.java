...
public abstract class Sender {
    ...
    throws XPathException {
        ...
        List<ProxyReceiver> filters = options.getFilters();
        if (filters != null) {
            for (int i=filters.size()-1; i>=0; i--) {
                ProxyReceiver filter = filters.get(i);
...
//        receiver = tf;
    ...
    private static Receiver makeValidator(Receiver receiver, String systemId, ParseOptions options) {
        PipelineConfiguration pipe = receiver.getPipelineConfiguration();
        Configuration config = pipe.getConfiguration();
        int schemaValidation = options.getSchemaValidationMode();
