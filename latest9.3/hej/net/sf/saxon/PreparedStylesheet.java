...
public class PreparedStylesheet extends Executable implements Templates, Serializable {
    ...
    // a string explaining why this stylesheet can't be compiled, or null if it can
    ...
    protected PreparedStylesheet(Configuration config, CompilerInfo info) {
        ...
        setCollationTable(new CollationMap(config.getCollationMap()));
    ...
    protected void setStylesheetDocument(DocumentImpl doc)
        ...
        if (getStyleNodeFactory().getXsltProcessorVersion().compareTo(DecimalValue.TWO) > 0 &&
                !getConfiguration().isLicensedFeature(Configuration.LicenseFeature.PROFESSIONAL_EDITION)) {
            try {
                ...
                XPathException w = new XPathException(
                        "Processing an XSLT 3.0 stylesheet requires Saxon-PE or Saxon-EE");
                w.setLocator(topnode);
                getConfiguration().getErrorListener().fatalError(w);
                throw w;
            } catch (TransformerException e) {
                throw XPathException.makeXPathException(e);
            }
        ...
        FunctionLibraryList fll = psm.createFunctionLibrary();
     ...
     *
     * @return null if the stylesheet can be compiled, or a message otherwise
     */
    ...
    public static PreparedStylesheet loadCompiledStylesheet(String fileName, Configuration config)
            throws IOException, ClassNotFoundException {
        ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fileName));
        PreparedStylesheet sheet = (PreparedStylesheet)ois.readObject();
        ois.close();
        NamePool compiledNamePool = sheet.getConfiguration().getNamePool();
