...
public abstract class AtomicValue extends Value implements Item, GroundedValue, ConversionResult {
    ...
    public final AtomicValue convert(AtomicType schemaType, ConversionRules rules) throws XPathException {
        return convert(schemaType, true, rules).asAtomic();
    }
    ...
    public final AtomicValue convert(AtomicType schemaType, XPathContext context) throws XPathException {
        ...
        return convert(schemaType, true, context.getConfiguration().getConversionRules()).asAtomic();
    }
    ...
    protected abstract ConversionResult convertPrimitive(
            BuiltInAtomicType requiredType, boolean validate, ConversionRules rules);
    ...
    public final ConversionResult convert(AtomicType targetType, boolean validate, ConversionRules rules) {
        if (targetType.isPrimitiveType()) {
            return convertPrimitive((BuiltInAtomicType)targetType, validate, rules);
            ...
            ConversionResult cr = convertPrimitive(primitiveType, validate, rules);
            ...
            if (primitiveType.getFingerprint() == StandardNames.XS_STRING) {
                ...
                lexicalValue = getCanonicalLexicalRepresentation(); //TODO: this is expensive and usually unnecessary
            }
            ValidationFailure vf = targetType.validate((AtomicValue)cr, lexicalValue, rules);
            if (vf != null) {
                return vf;
            }
