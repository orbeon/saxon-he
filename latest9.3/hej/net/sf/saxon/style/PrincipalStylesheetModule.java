...
//                }
    ...
    protected int getNamespaceAlias(short uriCode) {
        int alias = namespaceAliasMap.get(uriCode);
        return (alias == namespaceAliasMap.getDefaultValue() ? uriCode : alias);
    }
    ...
    protected boolean isAliasResultNamespace(short uriCode) {
        return aliasResultUriSet.contains(uriCode);
    }
    ...
    private void collectNamespaceAliases() throws XPathException {
        namespaceAliasMap = new IntToIntHashMap(numberOfAliases);
        aliasResultUriSet = new IntHashSet(numberOfAliases);
        ...
        for (int i = 0; i < numberOfAliases; i++) {
            ...
            if (aliasesAtThisPrecedence.contains(scode)) {
                if (namespaceAliasMap.get(scode) != (ncode&0xffff)) {
                    xna.compileError("More than one alias is defined for the same namespace", "XTSE0810");
                }
            }
            if (namespaceAliasMap.get(scode) == namespaceAliasMap.getDefaultValue()) {
                namespaceAliasMap.put(scode, ncode);
                aliasResultUriSet.add(ncode&0xffff);
            }
...
//            }
            ...
            exec.setStripsInputTypeAnnotations(getInputTypeAnnotations() == XSLStylesheet.ANNOTATION_STRIP);
            ...
            for (Iterator<FunctionLibrary> iter = functionLibrary.getLibraryList().iterator(); iter.hasNext();) {
                FunctionLibrary lib = iter.next();
    ...
    public boolean stripsWhitespace() {
        for (int i = 0; i < topLevel.size(); i++) {
            NodeInfo s = topLevel.get(i).getSourceElement();
