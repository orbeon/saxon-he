...
public class XQueryFunction implements InstructionInfo, Container, Declaration {
    ...
    public SymbolicName getIdentificationKey() {
        return new SymbolicName(StandardNames.XSL_FUNCTION, functionName, arguments.size());
    }
    ...
    public static SymbolicName getIdentificationKey(StructuredQName qName, int arity) {
        return new SymbolicName(StandardNames.XSL_FUNCTION, qName, arity);
    }
    ...
    public void compile() throws XPathException {
        ...
        try {
            ...
            if (compiledFunction == null) {
                ...
                body = visitor.typeCheck(body, ContextItemStaticInfo.ABSENT);
        ...
        if (opt.getOptimizationLevel() != Optimizer.NO_OPTIMIZATION) {
            ...
            GlobalVariableManager manager = new GlobalVariableManager() {
                ...
                public Collection<StructuredQName> getGlobalVariableNames() {
                    return exec.getGlobalVariableMap().getVariableMap();
                }

                public GlobalVariable getGlobalVariable(StructuredQName name) {
                    // TODO: PERF: avoid sequential search
                    for (GlobalVariable var : exec.getCompiledGlobalVariables()) {
                        if (var.getVariableQName().equals(name)) {
                            return var;
                        }
                    }
                    return null;
                ...
                public void addGlobalVariable(GlobalVariable variable) throws XPathException {
                    PackageData pd = new PackageData(exec.getConfiguration());
                    pd.setHostLanguage(Configuration.XQUERY);
                    pd.setAllowXPath30(exec.isAllowXPath30());
                    pd.setLocationMap(exec.getLocationMap());
                    pd.setSchemaAware(exec.isSchemaAware());
                    variable.setPackageData(pd);
                    exec.getCompiledGlobalVariables().add(variable);
                    SlotManager sm = exec.getGlobalVariableMap();
            ...
            if (b2 != null) {
                body = visitor.optimize(b2, ContextItemStaticInfo.ABSENT);
            }
