...
public class SequenceExtent implements GroundedValue {
    ...
    public ItemType getItemType(/*@Nullable*/ TypeHierarchy th) {
        ...
        if (it != null) {
            ...
            for (Item aValue : value) {
                if (aValue instanceof NodeInfo) {
                    it = getItemType(((NodeInfo) aValue).getConfiguration().getTypeHierarchy());
                }
            }
        }
        if (it == null) {
            it = Type.getItemType(value.get(0), th);
            for (int i = 1; i < len; i++) {
                if (it == AnyItemType.getInstance()) {
                    // make a quick exit
                    return it;
                }
                it = Type.getCommonSuperType(it, Type.getItemType(value.get(i), th), th);
            }
        }
        return itemType = it;
    }
