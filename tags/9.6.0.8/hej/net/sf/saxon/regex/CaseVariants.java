...
public class CaseVariants {
    ...
    static void build() {
        ...
        DocumentInfo doc;
        try {
            doc = config.buildDocument(new StreamSource(in, "casevariants.xml"), options);
        } catch (XPathException e) {
            throw new RuntimeException("Failed to build casevariants.xml", e);
        }

        AxisIterator iter = doc.iterateAxis(AxisInfo.DESCENDANT, new NameTest(Type.ELEMENT, "", "c", doc.getNamePool()));
        while (true) {
