...
public class QueryAPIExamples {
    ...
    public static void exampleWithParam() throws XPathException {
        ...
        final DynamicQueryContext dynamicContext = new DynamicQueryContext(config);
        dynamicContext.setParameter(new StructuredQName("","","in"), Int64Value.makeIntegerValue(17));
    ...
    public static void examplePipeline() throws XPathException {
        ...
        // Run the first query
        final DynamicQueryContext dynamicContext = new DynamicQueryContext(config);
        dynamicContext.setParameter(new StructuredQName("","","in"), Int64Value.makeIntegerValue(3));
    ...
    public static void exampleDirectFunction() throws XPathException {
        ...
        final UserFunction fn1 = exp1.getMainModule().getUserDefinedFunction("f.ns", "t1", 2);
        if (fn1 == null) {
            throw new IllegalStateException("Function f:t1() not found");
        }
        final Controller controller = exp1.newController(new DynamicQueryContext(config));
        final Sequence[] arglist = new Sequence[2];
        arglist[0] = new Int64Value(10);
        for (int i = 3; i < 10; i++) {
            arglist[1] = new Int64Value(i);
            final Sequence result = fn1.call(arglist, controller);
