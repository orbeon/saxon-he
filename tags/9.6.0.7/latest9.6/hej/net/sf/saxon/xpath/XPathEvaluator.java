...
public class XPathEvaluator implements XPath {
     ...
     */
    /*@NotNull*/
    public XPathExpression compile(/*@Nullable*/ String expr) throws XPathExpressionException {
        if (expr == null) {
            throw new NullPointerException("expr");
        }
        try {
            Executable exec = new Executable(getConfiguration());
            SimpleContainer container = new SimpleContainer(new PackageData(getConfiguration()));
            Expression exp = ExpressionTool.make(expr, staticContext, container, 0, -1, 1, null);
            ExpressionVisitor visitor = ExpressionVisitor.make(staticContext);
            final ContextItemStaticInfo contextItemType = new ContextItemStaticInfo(Type.ITEM_TYPE, true);
