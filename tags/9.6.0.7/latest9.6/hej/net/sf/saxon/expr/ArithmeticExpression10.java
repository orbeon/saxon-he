...
public class ArithmeticExpression10 extends BinaryExpression implements Callable {
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        ...
        if (Literal.isEmptySequence(operand0)) {
            return Literal.makeLiteral(DoubleValue.NaN, getContainer());
        }
        ...
        type0 = (AtomicType) operand0.getItemType().getPrimitiveItemType();
        ...
        type1 = (AtomicType) operand1.getItemType().getPrimitiveItemType();
        ...
        if (operator == Token.NEGATE) {
            ...
            return visitor.typeCheck(ne, contextInfo);
        }
        ...
        try {
            if ((operand0 instanceof Literal) && (operand1 instanceof Literal)) {
                return Literal.makeLiteral(
                        SequenceTool.toGroundedValue(evaluateItem(visitor.getStaticContext().makeEarlyEvaluationContext())), getContainer());
            }
    ...
    private Expression createConversionCode(
        ...
        if (th.isSubType(type, BuiltInAtomicType.BOOLEAN) ||
            ...
            if (operand instanceof Literal) {
                ...
                Literal lit = Literal.makeLiteral(NumberFn.convert((AtomicValue) val, config), getContainer());
                lit.setContainer(getContainer());
                return lit;
            } else {
                Expression exp = SystemFunctionCall.makeSystemFunction("number", new Expression[]{operand});
