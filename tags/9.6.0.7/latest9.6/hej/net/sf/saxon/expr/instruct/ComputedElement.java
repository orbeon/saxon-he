...
public class ComputedElement extends ElementCreator {
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        elementName = visitor.typeCheck(elementName, contextInfo);
        ...
        if (allowNameAsQName) {
            ...
            ItemType supplied = elementName.getItemType();
        ...
        if (namespace != null) {
            namespace = visitor.typeCheck(namespace, contextInfo);
            // following code deleted MHK Aug 2014, I think there are no circumstances in which this type check can fail
            // as the namespace expression is always an AVT and therefore always a string.
...
//                    namespace, SequenceType.SINGLE_STRING, false, role, visitor);
            ...
            try {
                ...
                if (val instanceof StringValue) {
                    ...
                    if (namespace == null) {
                        ...
                        namespace = new StringLiteral(uri, getContainer());
                    }
                }
    ...
    public Iterable<Operand> operands() {
        List<Operand> list = new ArrayList<Operand>(3);
        list.add(new Operand(content, OperandRole.SINGLE_ATOMIC));
        list.add(new Operand(elementName, OperandRole.SINGLE_ATOMIC));
        if (namespace != null) {
            list.add(new Operand(namespace, OperandRole.SINGLE_ATOMIC));
        }
        if (getOnEmpty() != null) {
            list.add(new Operand(onEmpty, OperandRole.NAVIGATE));
        }
        return list;
    }
