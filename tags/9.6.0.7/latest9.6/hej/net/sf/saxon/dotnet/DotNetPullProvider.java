...
public class DotNetPullProvider implements PullProvider, SourceLocator, SourceLocationProvider {
        ...
        if (parser.get_HasAttributes()) {
            ...
            for (int i = 0; i < parser.get_AttributeCount(); i++) {
                ...
                if ("xmlns".equals(prefix) || ("".equals(prefix) && "xmlns".equals(localName))) {
                    ...
                    atts.addAttribute(nc, BuiltInAtomicType.UNTYPED_ATOMIC, parser.get_Value(), 0, 0);
                }
            }
            return atts;
        } else {
    ...
    public NamespaceBinding[] getNamespaceDeclarations() throws XPathException {
        if (parser.get_HasAttributes()) {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                } else if (prefix.length() == 0 && "xmlns".equals(localName)) {
