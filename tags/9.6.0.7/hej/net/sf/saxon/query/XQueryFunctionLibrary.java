...
public class XQueryFunctionLibrary implements FunctionLibrary, XQueryFunctionBinder {
    ...
    /*@NotNull*/ private HashMap<SymbolicName, XQueryFunction> functions =
            new HashMap<SymbolicName, XQueryFunction>(20);
...
//#ifdefined HOF
    ...
    public FunctionItem getFunctionItem(SymbolicName functionName, StaticContext staticContext, Container container)
        ...
        if (fd != null) {
            ...
            FunctionItemType type = new com.saxonica.functions.hof.SpecificFunctionType(
                    fd.getArgumentTypes(), fd.getResultType());
            if (fn == null) {
                // not yet compiled
                UnresolvedCallable uc = new UnresolvedCallable(functionName);
                fd.registerReference(uc);
                return new com.saxonica.functions.hof.CallableFunctionItem(functionName, uc, type);
            } else {
                return new com.saxonica.functions.hof.CallableFunctionItem(fn);
            }
        } else {
            return null;
        }
...
//#endif
    ...
    /*@Nullable*/
    public Expression bind(/*@NotNull*/ SymbolicName functionName, /*@NotNull*/  Expression[] arguments, StaticContext env, Container container) throws XPathException {
        XQueryFunction fd = functions.get(functionName);
        if (fd != null) {
