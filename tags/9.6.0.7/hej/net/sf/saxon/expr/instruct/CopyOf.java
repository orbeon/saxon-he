...
import com.saxonica.ee.bytecode.CopyOfCompiler;
...
public class CopyOf extends Instruction implements ValidatingInstruction {
    ...
    static void copyAttribute(NodeInfo source,
        ...
        SimpleType annotation = validateAttribute(source, schemaType, validation, context.getConfiguration());
        try {
            context.getReceiver().attribute(new NameOfNode(source), annotation, value, instruction.getLocationId(), opt);
        } catch (XPathException e) {
            e.maybeSetContext(context);
            e.maybeSetLocation(instruction);
    ...
    public static SimpleType validateAttribute(
            NodeInfo source, SimpleType schemaType, int validation, Configuration config) throws XPathException {
        CharSequence value = source.getStringValueCS();
        ...
        if (schemaType != null) {
            ...
            try {
                ValidationFailure err = schemaType.validateContent(
                        value, DummyNamespaceResolver.getInstance(), config.getConversionRules());
                if (err != null) {
            ...
            try {
                annotation = config.validateAttribute(source.getNameCode(), value, validation);
    ...
    public SequenceIterator iterate(XPathContext context) throws XPathException {
        ...
        if (schemaType == null && copyNamespaces && !copyForUpdate) {
            if (validation == Validation.PRESERVE) {
                ...
                return new ItemMappingIterator<Item, Item>(select.iterate(context), copier, true);
