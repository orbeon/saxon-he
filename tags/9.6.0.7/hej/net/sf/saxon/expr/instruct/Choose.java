...
public class Choose extends Instruction {
    ...
    private Expression removeRedundantBranches(ExpressionVisitor visitor) {
        ...
        for (int i = 0; i < conditions.length; i++) {
            if (Literal.isConstantBoolean(conditions[i], false)) {
                if (conditions.length == 1) {
                    Literal lit = Literal.makeEmptySequence(getContainer());
        ...
        if (conditions.length == 2 &&
            ...
            if (th.isSubType(conditions[0].getItemType(), BuiltInAtomicType.BOOLEAN) &&
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        for (int i = 0; i < conditions.length; i++) {
            conditions[i] = visitor.typeCheck(conditions[i], contextInfo);
        ...
        for (int i = 0; i < actions.length; i++) {
            try {
                actions[i] = visitor.typeCheck(actions[i], contextInfo);
            } catch (XPathException err) {
    ...
    public Expression staticTypeCheck(SequenceType req,
        ...
        for (int i = 0; i < actions.length; i++) {
            try {
                ...
                ExpressionTool.copyLocationInfo(actions[1], ee);
                actions[i] = ee;
            }
        }
        ...
        if (!Literal.isConstantBoolean(conditions[conditions.length - 1], true) &&
            ...
            c[conditions.length] = Literal.makeLiteral(BooleanValue.TRUE, getContainer());
    ...
    public Expression optimize(ExpressionVisitor visitor, ContextItemStaticInfo contextItemType) throws XPathException {
        for (int i = 0; i < conditions.length; i++) {
            ...
            if (conditions[i] instanceof Literal &&
                ...
                conditions[i] = Literal.makeLiteral(BooleanValue.get(b), getContainer());
            }
        }
        for (int i = 0; i < actions.length; i++) {
            try {
    ...
    public ItemType getItemType() {
        ...
        for (int i = 1; i < actions.length; i++) {
            type = Type.getCommonSuperType(type, actions[i].getItemType(), th);
        }
        return type;
    }
    ...
    public Iterable<Operand> operands() {
        List<Operand> list = new ArrayList<Operand>(conditions.length*2);
        for (Expression c : conditions) {
            list.add(new Operand(c, OperandRole.INSPECT));
        }
        for (Expression c : actions) {
    ...
    public Expression unordered(boolean retainAllNodes, boolean forStreaming) throws XPathException {
        for (int i = 0; i < actions.length; i++) {
            actions[i] = actions[i].unordered(retainAllNodes, forStreaming);
        }
        return this;
    }
    ...
    public String toShortString() {
        return "if(" + conditions[0].toShortString() + ") then ... else ...";
    }
    ...
    public SequenceIterator iterate(XPathContext context) throws XPathException {
        int i = choose(context);
        return (i < 0 ? EmptyIterator.emptyIterator() : actions[i].iterate(context));
    }
    ...
    public EventIterator iterateEvents(XPathContext context) throws XPathException {
        int i = choose(context);
        return (i < 0 ? EmptyEventIterator.getInstance() : actions[i].iterateEvents(context));
    }
