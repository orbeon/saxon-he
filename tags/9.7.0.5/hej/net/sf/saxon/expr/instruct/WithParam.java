...
public class WithParam  {
    ...
    public static void promoteParams(WithParam[] params, PromotionOffer offer) throws XPathException {
        if (params != null) {
            for (WithParam param : params) {
                Operand selectOp = param.selectOp;
                if (selectOp != null) {
                    param.selectOp.setChildExpression(selectOp.getChildExpression().promote(offer));
                    param.computeEvaluationMode();
                }
            }
        }
    ...
    private void computeEvaluationMode() {
        evaluationMode = ExpressionTool.lazyEvaluationMode(selectOp.getChildExpression());
    }
    ...
    public Sequence getSelectValue(XPathContext context) throws XPathException {
        ...
        Sequence result = ExpressionTool.evaluate(selectOp.getChildExpression(), evaluationMode, context, 10);
