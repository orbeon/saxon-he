...
public class XPathDynamicContext {
    ...
    public void setContextItem(Item item) throws XPathException {
        if (item instanceof FingerprintedNode) {
            if (!((FingerprintedNode) item).getConfiguration().isCompatible(contextObject.getConfiguration())) {
    ...
    public void setVariable(XPathVariable variable, Sequence value) throws XPathException {
        ...
        while ((item = iter.next()) != null) {
            if (item instanceof FingerprintedNode && !((FingerprintedNode) item).getConfiguration().isCompatible(contextObject.getConfiguration())) {
                throw new XPathException(
                        "Supplied node must be built using the same or a compatible Configuration",
                        SaxonErrorCode.SXXP0004);
            }
    ...
    public void setErrorListener(ErrorListener listener) {
        contextObject.setErrorListener(listener);
    }
    ...
    public UnfailingErrorListener getErrorListener() {
        return contextObject.getErrorListener();
    }
