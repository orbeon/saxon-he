...
public class NameOfNode implements NodeName {
    ...
    /**
     * Get the nameCode of this name if known. This method should not do any work to allocate
    ...
    public int getNameCode() {
        if (hasFingerprint()) {
            return ((FingerprintedNode) node).getNameCode();
        } else {
            return -1;
        }
    }
    ...
    public int allocateNameCode(NamePool namePool) {
        if (node instanceof FingerprintedNode) {
            ...
            return namePool.allocate(node.getPrefix(), node.getURI(), node.getLocalPart());
        }
    }
    ...
    public boolean equals(Object obj) {
        if (obj instanceof NodeName) {
            ...
            if (node instanceof FingerprintedNode && n.hasFingerprint()) {
                return ((FingerprintedNode)node).getFingerprint() == n.getFingerprint();
            } else {
