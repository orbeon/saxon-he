...
public class ImportedFunctionLibrary implements FunctionLibrary, XQueryFunctionBinder {
    ...
    private HashSet<String> namespaces = new HashSet<String>(5);
    ...
    /*@Nullable*/
    public Expression bind(/*@NotNull*/ SymbolicName symbolicName, Expression[] staticArgs, StaticContext env) throws XPathException {
        ...
        for (Expression arg : staticArgs) {
            if (arg.getRetainedStaticContext() == null) {
                arg.setRetainedStaticContext(rsc);
            }
        }
        if (namespaces.contains(uri)) {
            Expression call = baseLibrary.bind(symbolicName, staticArgs, env);
            if (call != null) {
                // Check that the result type and all the argument types are in the static context of the
                // calling module
                XQueryFunction def = baseLibrary.getDeclaration(functionName, staticArgs.length);
