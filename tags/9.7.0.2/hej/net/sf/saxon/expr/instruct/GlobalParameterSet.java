...
public class GlobalParameterSet {
    private HashMap<StructuredQName, Sequence> params =
            new HashMap<StructuredQName, Sequence>(10);
    ...
    public void put(StructuredQName qName, /*@Nullable*/ Sequence value) {
        if (value == null) {
            params.remove(qName);
        } else {
        ...
        if (requiredType != null) {
            if (convert) {
                ...
                val = config.getTypeHierarchy().applyFunctionConversionRules(
                    val, requiredType, role, ExplicitLocation.UNKNOWN_LOCATION);
            } else {
