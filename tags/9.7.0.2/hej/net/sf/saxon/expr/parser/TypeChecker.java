...
//            }
        ...
        if (backwardsCompatible && !allowsMany) {
            ...
            if (!itemTypeOK) {
                ...
                if (reqItemType.equals(BuiltInAtomicType.STRING)) {
                    ...
                    Expression fn = SystemFunction.makeCall("string", rsc, exp);
                    try {
                        exp = fn.simplify().typeCheck(visitor, ContextItemStaticInfo.DEFAULT);
                ...
                if (reqItemType.equals(NumericType.getInstance()) || reqItemType.equals(BuiltInAtomicType.DOUBLE)) {
                    final RetainedStaticContext rsc = new RetainedStaticContext(config);
                    Expression fn = SystemFunction.makeCall("number", rsc, exp);
                    try {
                        exp = fn.simplify().typeCheck(visitor, ContextItemStaticInfo.DEFAULT);
                    } catch (XPathException err) {
                        err.maybeSetLocation(exp.getLocation());
            ...
            if (reqItemType.isPlainType()) {
                ...
                if (!suppliedItemType.isPlainType() &&
                    ...
                    if (!suppliedItemType.isAtomizable()) {
                        XPathException err = new XPathException("An atomic value is required for the " + role.getMessage() +
                            ", but the supplied value cannot be atomized", "FOTY0013", supplied.getLocation());
                ...
                if (suppliedItemType.equals(BuiltInAtomicType.UNTYPED_ATOMIC)
                    ...
                    Expression cexp = UntypedSequenceConverter.makeUntypedSequenceConverter(config, exp, (PlainType) reqItemType);
                    ExpressionTool.copyLocationInfo(exp, cexp);
                    try {
                ...
                if (suppliedItemType.equals(BuiltInAtomicType.ANY_ATOMIC)
                    ...
                    if (((SimpleType) reqItemType).isNamespaceSensitive()) {
                        ...
                        conversion = UntypedSequenceConverter.makeUntypedSequenceConverter(config, exp, (PlainType) reqItemType);
                    }
                ...
                if (reqItemType instanceof AtomicType) {
                    ...
                    if (rt == StandardNames.XS_DOUBLE &&
                            th.relationship(suppliedItemType, NumericType.getInstance()) != TypeHierarchy.DISJOINT) {
                        Expression cexp = makePromoterToDouble(exp);
                        ExpressionTool.copyLocationInfo(exp, cexp);
                        exp = cexp;
                        try {
                            exp = exp.simplify().typeCheck(visitor, ContextItemStaticInfo.DEFAULT);
                        } catch (XPathException err) {
                            err.maybeSetLocation(exp.getLocation());
                            err.setIsStaticError(true);
                            throw err;
        ...
        while ((item = iter.next()) != null) {
            ...
            if (!reqItemType.matches(item, context.getConfiguration().getTypeHierarchy())) {
                XPathException err = new XPathException("Required type is " + reqItemType +
                        "; supplied value has type " + UType.getUType(SequenceTool.toGroundedValue(val)));
                err.setIsTypeError(true);
    ...
    public static XPathException ebvError(Expression exp, TypeHierarchy th) {
        ...
        if (th.relationship(t, Type.NODE_TYPE) == TypeHierarchy.DISJOINT &&
                ...
                th.relationship(t, NumericType.getInstance()) == TypeHierarchy.DISJOINT &&
    ...
    private static Expression makePromoterToDouble(Expression exp) {
        ...
        converter.setConversionRules(exp.getConfiguration().getConversionRules());
