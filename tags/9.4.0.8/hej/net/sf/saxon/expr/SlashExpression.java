...
public class SlashExpression extends Expression
    ...
    private SlashExpression simplifyDescendantPath(StaticContext env) {
        ...
        if (underlyingAxis == Axis.CHILD ||
                underlyingAxis == Axis.DESCENDANT ||
                underlyingAxis == Axis.DESCENDANT_OR_SELF) {
            byte newAxis = (underlyingAxis == Axis.DESCENDANT_OR_SELF ? Axis.DESCENDANT_OR_SELF : Axis.DESCENDANT);
            Expression newStep =
        ...
        if (underlyingAxis == Axis.ATTRIBUTE) {
