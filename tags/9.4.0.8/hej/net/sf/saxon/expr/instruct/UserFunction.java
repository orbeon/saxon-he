...
public class UserFunction extends Procedure {
    ...
    /*@Nullable*/ public Boolean isInlineable() {
        if (inlineable < 0) {
            return null;
        } else {
            return inlineable == 1;
    ...
    private void gatherDirectContributingCallees(Expression exp, Set<UserFunction> result) {
        ...
        while (kids.hasNext()) {
            ...
            if (kid instanceof UserFunctionCall) {
                ...
                for (Expression action : actions) {
                    gatherDirectContributingCallees(action, result);
                }
