            ...
            if (val != Validation.PRESERVE) {
                ...
                if (topLevelName != null) {
                    topLevelNameCode = config.getNamePool().allocate(
                        topLevelName.getPrefix(), topLevelName.getURI(), topLevelName.getLocalPart());
                }
...
//        receiver = tf;
    ...
    private static Receiver makeValidator(Receiver receiver, String systemId, ParseOptions options) throws XPathException {
        ...
        Configuration config = pipe.getConfiguration();
        int schemaValidation = options.getSchemaValidationMode();
        int sv = schemaValidation  & Validation.VALIDATION_MODE_MASK;
        if (sv != Validation.PRESERVE && sv != Validation.DEFAULT) {
            Controller controller = pipe.getController();
            if (controller != null && !controller.getExecutable().isSchemaAware() &&
                    (schemaValidation & Validation.VALIDATION_MODE_MASK) != Validation.STRIP) {
            ...
            if (topLevelElement != null) {
                topLevelElementCode = config.getNamePool().allocate(
                        topLevelElement.getPrefix(), topLevelElement.getURI(), topLevelElement.getLocalPart());
            }
