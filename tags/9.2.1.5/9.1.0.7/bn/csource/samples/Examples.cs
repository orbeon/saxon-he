...
public class Examples
{
    ...
    public static void Main(String[] argv)
    {
        ...
        {
            if (test == "all" || (test == "B" && !ex.testName.StartsWith("SA-")) || test == ex.testName)
            {
                Console.WriteLine("\n\n===== " + ex.testName + " =======\n");
                try
                {
                ...
                {
                    Console.WriteLine("Test failed with dynamic error: " + se.Message);
                }
                catch (Saxon.Api.DynamicError de)
                {
                    Console.WriteLine("Test failed with dynamic error: " + de.Message);
                }
                catch (Exception exc)
                {
                    Console.WriteLine("Test failed unexpectedly: " + exc.Message);
...
public class XsltCapturingErrors : Example
{
    ...
    /// <summary>
    /// RRun an XSLT transformation capturing compile-time errors within the application
    /// </summary>
...
public class XsltCapturingMessages : Example
{
    ...
    /// <summary>
    /// RRun an XSLT transformation capturing run-time messages within the application
    /// </summary>
...
public class XsltExtensibility : Example
{
    ...
    public override void run(Uri samplesDir)
    {
        ...
        String s = @"<xsl:transform version='2.0' xmlns:xsl='http://www.w3.org/1999/XSL/Transform'" +
            ...
            @" <xsl:sequence select='ext:combine(ext:FirstChild((//ITEM)[1]), count(*))'/> " +
            @" </out> " +
...
public class Validate : Example
{
    ...
    public override void run(Uri samplesDir)
    {
        ...
        catch (Exception err)
        {
            ...
            Console.WriteLine("Failed to load Saxon-SA (use -B option to run Saxon-B tests only)");
            return;
        }
        ...
        Uri instanceUri = new Uri(samplesDir, "data/books.xml");
...
public class ExtraTestCase : Example
{
    ...
    public override void run(Uri samplesDir)
    {
        Console.WriteLine("Extra test case: does nothing");
    }
