            ...
            return new PJConverter() {
                public Object convert(ValueRepresentation<? extends Item> value, Class targetClass, XPathContext context) throws XPathException {
                    if (value instanceof SingletonItem) {
                        NodeInfo node = (NodeInfo)((SingletonItem)value).asItem();
   ...
   /**
    ...
    public Value convertObjectToXPathValue(Object object, Configuration config) throws XPathException {
        if (object instanceof XmlNode) {
            ...
            return new SingletonItem<NodeWrapper>(nw);
    ...
    public Object convertXPathValueToObject(Value value, cli.System.Type targetClass) throws XPathException {
        //System.err.println("CONVERT TO: " + targetClass.toString());
        if (isXmlNodeType(targetClass) &&
                value instanceof SingletonItem &&
                ((SingletonItem)value).asItem() instanceof NodeWrapper) {
            return ((NodeWrapper)((SingletonItem)value).asItem()).getRealNode();
        } else if (isXdmAtomicValueType(targetClass)) {
            SequenceIterator atomIterator = Atomizer.getAtomizingIterator(value.iterate(), false);
            ValueRepresentation extent = SequenceExtent.makeSequenceExtent(atomIterator);
            if (extent instanceof EmptySequence) {
                return null;
            } else if (extent instanceof AtomicValue) {
                return wrapAsXdmValue((AtomicValue)extent);
            } else {
    ...
    /*@Nullable*/ public NodeInfo wrapNode(DocumentInfo document, Object node) {
