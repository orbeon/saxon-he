...
public class PathMap {
    ...
    public static class PathMapArc {
        ...
        /**
         * Create a PathMapArc
         * @param axis the axis (a constant from class {@link Axis}
         ...
         * @return the axis, a constant from class {@link Axis}
         */
    ...
    public static class PathMapNodeSet extends HashSet<PathMapNode> {
       ...
       /**
        ...
        public void addDescendants() {
            for (PathMapNode node : this) {
                node.createArc(Axis.DESCENDANT, AnyNodeTest.getInstance());
            }
        }
            ...
            for (int i=root.arcs.size()-1; i>=0; i--) {
                ...
                switch (axis) {
                    ...
                    case Axis.NAMESPACE: {
                        ...
                        newRoot.createArc(Axis.DESCENDANT, NodeKindTest.ELEMENT, newTarget);
                        break;
                    }
                    default: {
                        newRoot.createArc(Axis.DESCENDANT_OR_SELF, arc.getNodeTest(), arc.getTarget());
                        break;
                    }
                }
            }
        ...
        for (int i=node.arcs.size()-1; i>=0; i--) {
            ...
            switch (thisArc.getAxis()) {
                ...
                case Axis.PRECEDING: {
                    ...
                    if (thisArc.getAxis() != Axis.DESCENDANT_OR_SELF) {
                        root.createArc(Axis.DESCENDANT_OR_SELF, thisArc.getNodeTest(), thisArc.getTarget());
                        node.arcs.remove(i);
                    }
                    break;
                }
                ...
                case Axis.PRECEDING_SIBLING: {
                    if (grandParent != null) {
                        ...
                        root.createArc(Axis.CHILD, thisArc.getNodeTest(), thisArc.getTarget());
                        node.arcs.remove(i);
                        break;
                    }
                }
                case Axis.PARENT: {
                    ...
                    if (lastAxis == Axis.CHILD || lastAxis == Axis.ATTRIBUTE || lastAxis == Axis.NAMESPACE) {
                        ...
                        if (thisArc.getTarget().arcs.isEmpty()) {
                            grandParent.createArc(Axis.DESCENDANT_OR_SELF, thisArc.getNodeTest());
                        } else {
                            grandParent.createArc(Axis.DESCENDANT_OR_SELF, thisArc.getNodeTest(), thisArc.getTarget());
                        }
                        node.arcs.remove(i);
                    } else {
                        // don't try to be precise about a/b/../../c
                        if (thisArc.getTarget().arcs.isEmpty()) {
                            root.createArc(Axis.DESCENDANT_OR_SELF, thisArc.getNodeTest());
                        } else {
                            root.createArc(Axis.DESCENDANT_OR_SELF, thisArc.getNodeTest(), thisArc.getTarget());
                        }
                        node.arcs.remove(i);
    ...
    private void showArcs(/*@NotNull*/ PrintStream out, /*@NotNull*/ PathMapNode node, int indent) {
        ...
        for (PathMapArc arc : arcs) {
            out.println(pad + Axis.axisName[arc.axis] +
                    "::" +
                    arc.test.toString() +
