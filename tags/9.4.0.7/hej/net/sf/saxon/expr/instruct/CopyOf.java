...
public class CopyOf extends Instruction implements ValidatingInstruction {
            ...
            if (item instanceof NodeInfo) {
                ...
                switch (kind) {
                    ...
                    case Type.ELEMENT: {
                        Receiver eval = out;
                        if (validating) {
                            eval = controller.getConfiguration().getElementValidator(out, new NameOfNode(source), locationId, schemaType, validation);
                        }
                        if (copyBaseURI) {
    ...
    static void copyAttribute(NodeInfo source,
        ...
        if (schemaType != null) {
            if (schemaType.isSimpleType()) {
                ...
                e.setXPathContext(context);
                e.setErrorCode("XTTE1535"); // See spec bug 13001 (anticipating acceptance of the proposed resolution)
    ...
    public SequenceIterator<? extends Item> iterate(XPathContext context) throws XPathException {
        ...
        return Value.asIterator(out.getSequence());
    }
