...
public class ForEachGroup extends Instruction
    ...
    public ForEachGroup(Expression select,
        ...
        keyOp = new Operand(this, key, OperandRole.NEW_FOCUS_ATOMIC);
    ...
    public Expression optimize(ExpressionVisitor visitor, ContextItemStaticInfo contextItemType) throws XPathException {
        ...
        ItemType selectedItemType = getSelectExpression().getItemType();
        ContextItemStaticInfo sit = new ContextItemStaticInfo(selectedItemType, false, getSelectExpression());
        ...
        if (collator == null && (getCollationNameExpression() instanceof StringLiteral)) {
            ...
            try {
                ...
                if (!collationURI.isAbsolute()) {
                    ...
                    collator = getConfiguration().getCollation(collationNameString);
                    if (collator == null) {
    ...
    protected void promoteChildren(PromotionOffer offer) throws XPathException {
        setSelect(doPromotion(getSelectExpression(), offer));
        if (offer.action == PromotionOffer.EXTRACT_GLOBAL_VARIABLES) {
            // Don't pass on other requests
            setAction(doPromotion(getActionExpression(), offer));
            setKey(doPromotion(getGroupingKey(), offer));
        }
    ...
    public GroupIterator getGroupIterator(FocusIterator population, XPathContext context) throws XPathException {
        ...
        switch (algorithm) {
            ...
            case GROUP_ADJACENT: {
                ...
                groupIterator = new GroupAdjacentIterator(population, getGroupingKey(), context, coll, composite);
                break;
            }
            case GROUP_STARTING:
                groupIterator = new GroupStartingIterator(population, (Pattern) getGroupingKey(), context);
                break;
            case GROUP_ENDING:
                groupIterator = new GroupEndingIterator(population, (Pattern) getGroupingKey(), context);
                break;
            default:
