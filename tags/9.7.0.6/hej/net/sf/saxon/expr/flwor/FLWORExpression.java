...
public class FLWORExpression extends Expression {
    ...
    public Expression simplify() throws XPathException {
        OperandProcessor simplifier = new OperandProcessor() {
            public void processOperand(Operand op) throws XPathException {
    ...
    public Expression typeCheck(final ExpressionVisitor visitor, final ContextItemStaticInfo contextInfo)
        ...
        OperandProcessor typeChecker = new OperandProcessor() {
            public void processOperand(Operand op) throws XPathException {
    ...
    public Expression staticTypeCheck(SequenceType req,
        ...
        returnClauseOp.setChildExpression(
                TypeChecker.staticTypeCheck(getReturnClause(), req, backwardsCompatible, role, visitor));
        return this;
    }
    ...
    public Expression promote(final PromotionOffer offer) throws XPathException {
        OperandProcessor processor = new OperandProcessor() {
            /*@Nullable*/
            public void processOperand(Operand op) throws XPathException {
                op.setChildExpression(doPromotion(op.getChildExpression(), offer));
            }
        };
    ...
    public Expression optimize(
        ...
        for (Clause c : clauses) {
            c.processOperands(new OperandProcessor() {
                public void processOperand(Operand op) throws XPathException {
                    op.optimize(visitor, contextItemType);
                }
            });
    ...
    public void process(XPathContext context) throws XPathException {
        TuplePush destination = new ReturnClausePush(getReturnClause());
        for (int i = clauses.size() - 1; i >= 0; i--) {
    ...
    public boolean hasLoopingVariableReference(final Binding binding) {
        ...
        if (!ExpressionTool.dependsOnVariable(getReturnClause(), new Binding[]{binding})) {
            ...
            OperandProcessor checker = new OperandProcessor() {
                public void processOperand(Operand op) throws XPathException {
                    if (response.isEmpty() &&
