...
/**
 * Implementation of the extension function array:put(array, xs:integer, item()*) => array
 * Added to the spec by bug 29685
 */
public class ArrayPut extends ExtensionFunctionDefinition{

    private final static StructuredQName name = new StructuredQName("array", NamespaceConstant.ARRAY_FUNCTIONS, "put");
    private final static SequenceType[] ARG_TYPES = new SequenceType[]{
            ArrayItem.SINGLE_ARRAY_TYPE, SequenceType.SINGLE_INTEGER, SequenceType.ANY_SEQUENCE};
        ...
        return new ExtensionFunctionCall() {
            public Sequence call(XPathContext context, Sequence[] arguments) throws XPathException {
                ...
                if (index < 0 || index >= array.size()){
                    throw new XPathException("Specified position (" + (index+1) + ") is not in range", "FOAY0001");
                }
                Sequence newMember = arguments[2];
                List<Sequence> list = new ArrayList<Sequence>(1);
                int size = array.size();
                for (int i=0; i < size; i++) {
                    list.add(i==index ? newMember : array.get(i));
                }
