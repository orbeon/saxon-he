...
//        p1.verifyParentPointers();
    ...
    final public Iterable<Operand> operands() {
        return new Iterable<Operand>() {
            public Iterator<Operand> iterator() {
                return new PairIterator<Operand>(lhs, rhs);
            }
        };
    }
        ...
        if (other instanceof BinaryExpression) {
            ...
            if (operator == b.operator) {
                if (getLhsExpression().equals(b.getLhsExpression()) &&
                        getRhsExpression().equals(b.getRhsExpression())) {
                    return true;
                }
                if (isCommutative(operator) &&
                        getLhsExpression().equals(b.getRhsExpression()) &&
                        getRhsExpression().equals(b.getLhsExpression())) {
                    return true;
                }
                if (isAssociative(operator) &&
                        pairwiseEqual(flattenExpression(new ArrayList<Expression>(4)),
                                b.flattenExpression(new ArrayList<Expression>(4)))) {
                    return true;
                }
            }
    ...
    public String toShortString() {
        return getLhsExpression().toShortString() + " " + displayOperator() + " " + getRhsExpression().toShortString();
    }
