...
public abstract class ComponentCode<T extends ComponentCode> implements InstructionInfo {
    ...
    private Component<T> declaringComponent;
    ...
    public Component<T> makeDeclaringComponent(Visibility visibility, StylesheetPackage declaringPackage) {
        if (declaringComponent == null) {
            declaringComponent = new Component<T>((T)this, visibility, declaringPackage, declaringPackage);
        }
        return declaringComponent;
    }

    public Component<T> getDeclaringComponent() {
        return declaringComponent;
    }

    public void setDeclaringComponent(Component<T> comp) {
    ...
    protected static void allocateBindingSlotsRecursive(StylesheetPackage pack, ComponentCode p, Expression exp, List<ComponentBinding> bindings) {
    ...
    /**
     * Process a component reference from this component to some other component
     * @param pack the containing package
    ...
    private void processComponentReference(StylesheetPackage pack, ComponentInvocation invocation, List<ComponentBinding> bindings) {
        ...
        Component<T> target = (Component<T>)pack.getComponent(name);
        if (target == null) {
            target = (Component<T>)pack.getHiddenComponent(name);
        }
        if (target == null && name.getComponentName().hasURI(NamespaceConstant.XSLT) &&
                name.getComponentName().getLocalPart().equals("original")) {
            target = (Component<T>)pack.getOverriddenComponent(getSymbolicName());
        }
        if (target == null) {
