        ...
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (Float.isInfinite(quotient)) {
                ...
                return new DecimalIdivDecimal().compute(
                    new DecimalValue(((NumericValue) a).getDecimalValue()),
                    new DecimalValue(((NumericValue) b).getDecimalValue()),
                    c);
            } else {
                return Converter.FLOAT_TO_INTEGER.convert(new FloatValue(quotient)).asAtomic();
            }
        }
...
//                Math.max(A.scale(), B.scale()));
        int scale = Math.max(DecimalValue.DIVIDE_PRECISION, A.scale() - B.scale() + DecimalValue.DIVIDE_PRECISION);
