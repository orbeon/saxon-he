...
public final class CastableExpression extends CastingExpression {
    ...
    protected Expression preEvaluate() throws XPathException {
        ...
        if (literalOperand instanceof AtomicValue && converter != null) {
            ...
            return Literal.makeLiteral(BooleanValue.get(!(result instanceof ValidationFailure)), getContainer());
        }
        final int length = literalOperand.getLength();
        if (length == 0) {
            return Literal.makeLiteral(BooleanValue.get(allowsEmpty()), getContainer());
        }
        if (length > 1) {
            return Literal.makeLiteral(BooleanValue.FALSE, getContainer());
        }
