...
public class AnalyzeString extends Instruction {
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        ...
        regex = visitor.typeCheck(regex, contextInfo);
        adoptChildExpression(regex);
        flags = visitor.typeCheck(flags, contextInfo);
        adoptChildExpression(flags);
        if (matching != null) {
            matching = visitor.typeCheck(matching, new ContextItemStaticInfo(BuiltInAtomicType.STRING, false));
            adoptChildExpression(matching);
        }
        if (nonMatching != null) {
            nonMatching = visitor.typeCheck(nonMatching, new ContextItemStaticInfo(BuiltInAtomicType.STRING, false));
    ...
    public Expression optimize(ExpressionVisitor visitor, ContextItemStaticInfo contextItemType) throws XPathException {
        ...
        adoptChildExpression(flags);
        if (matching != null) {
            matching = matching.optimize(visitor, new ContextItemStaticInfo(BuiltInAtomicType.STRING, false));
            adoptChildExpression(matching);
        }
        if (nonMatching != null) {
            nonMatching = nonMatching.optimize(visitor, new ContextItemStaticInfo(BuiltInAtomicType.STRING, false));
            adoptChildExpression(nonMatching);
        }
    ...
    public ItemType getItemType() {
        if (matching != null) {
            if (nonMatching != null) {
                ...
                return Type.getCommonSuperType(matching.getItemType(), nonMatching.getItemType(), th);
            } else {
    ...
    public Iterable<Operand> operands() {
        ...
        list.add(new Operand(select, OperandRole.SINGLE_ATOMIC));
        list.add(new Operand(regex, OperandRole.SINGLE_ATOMIC));
        list.add(new Operand(flags, OperandRole.SINGLE_ATOMIC));
        if (matching != null) {
            list.add(new Operand(matching, ACTION));
        }
        if (nonMatching != null) {
            list.add(new Operand(nonMatching, ACTION));
        }
        return list;
    }

    public static final OperandRole ACTION = new OperandRole(OperandRole.USES_NEW_FOCUS | OperandRole.HIGHER_ORDER, OperandUsage.NAVIGATION);
    ...
    private RegexIterator getRegexIterator(XPathContext context) throws XPathException {
        ...
        if (re == null) {
            ...
            if ((!allow30features || !useXsltErrorCodes) && re.matches("")) {
