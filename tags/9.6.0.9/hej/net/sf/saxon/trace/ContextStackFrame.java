...
public abstract class ContextStackFrame {
    ...
    public static class BuiltInTemplateRule extends ContextStackFrame {
        public void print(Logger out) {
            out.error("  in built-in template rule");
        }
    }
    ...
    private static String displayContainer(/*@NotNull*/ Container container) {
        if (container instanceof ComponentBody) {
            StructuredQName name = ((ComponentBody) container).getObjectName();
            ...
            if (qName.hasURI(NamespaceConstant.SAXON_GENERATED_GLOBAL)) {
