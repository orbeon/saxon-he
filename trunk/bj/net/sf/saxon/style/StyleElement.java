    ...
    public XSLStylesheet getContainingStylesheet() {
        if (containingStylesheet == null) {
            if (this instanceof XSLStylesheet) {
                ...
                containingStylesheet = ((StyleElement)getParent()).getContainingStylesheet();
            }
        }
...
//            if (forwardsCompatibleModeIsEnabled()) {
//                compileWarning("Invalid pattern, ignored because in forwards-compatibility mode. " +
//                        err.getMessage(), err.getErrorCodeLocalPart());
//                return new NodeTestPattern(NoNodeTest.getInstance());
//            } else {
            compileError(err);
...
//            }
    ...
    protected void processVersionAttribute(String nc) throws XPathException {
        ...
        if (v != null) {
            AtomicValue val = DecimalValue.makeDecimalValue(v.trim(), true);
...
//            }
    ...
    public Pattern typeCheck(String name, Pattern pattern) throws XPathException {
        ...
        try {
            pattern = pattern.analyze(staticContext, Type.NODE_TYPE);
            boolean usesCurrent = false;
            int current = getNamePool().allocate("", NamespaceConstant.FN, "current") & NamePool.FP_MASK;
            if (pattern instanceof LocationPathPattern) {
                Iterator sub = pattern.iterateSubExpressions();
                while (sub.hasNext()) {
                    Expression filter = (Expression)sub.next();
                    if (ExpressionTool.callsFunction(filter, current)) {
                        usesCurrent = true;
                        break;
                    }
                }
                if (usesCurrent) {
                    ...
                    decl.setNameCode(config.getNamePool().allocate("saxon", NamespaceConstant.SAXON, "current" + hashCode()));
                    ...
                    let.setAction(EmptySequence.getInstance());
...
//            throws XPathException {
//        // the use of recursion in this method imposes a limit on the number of sibling instructions,
//        // which can prove a problem with software-generated stylesheets.
...
//                return text;
//            } else {
//                ComputedExpression e = (ComputedExpression)Block.makeBlock(text, tail);
...
//                return tail;
//            } else {
//                ComputedExpression e = (ComputedExpression)Block.makeBlock(child, tail);
//                e.setLocationId(allocateLocationId(getSystemId(), getLineNumber()));
//                return e;
//            }
//        }
...
//    }
    ...
    public Expression compileSequenceConstructor(Executable exec, AxisIterator iter, boolean includeParams)
        ...
        Expression result = EmptySequence.getInstance();
        ...
        while (true) {
            ...
            if (node.getNodeKind() == Type.TEXT) {
                ...
                if (result instanceof ComputedExpression) {
                    ((ComputedExpression)result).setLocationId(locationId);
                }
                continue;

            } else if (node instanceof XSLVariable) {
                Expression var = ((XSLVariable)node).compileLocalVariable(exec);
                if (var == null || var instanceof EmptySequence) {
                    // this can happen if the variable declaration is redundant
                    continue;
                } else {
                    LocalVariable lv = (LocalVariable)var;
                    Expression tail = compileSequenceConstructor(exec, iter, includeParams);
                    if (tail == null || tail instanceof EmptySequence) {
                        // this doesn't happen, because if there are no instructions following
                        // a variable, we'll have taken the var==null path above
                        return result;
                    } else {
                ...
                if (snode.validationError != null && !(this instanceof AbsentExtensionElement)) {
                    ...
                    if (child instanceof ComputedExpression) {
                        ...
                        childi.setLocationId(locationId);
                    }
                    if (child != null) {
                        if (includeParams || !(node instanceof XSLParam)) {
                            if (getConfiguration().isCompileWithTracing()) {
        ...
        while (true) {
            ...
            if (child instanceof XSLFallback) {
                ...
                if (fallback == null) {
                    ...
                    if (fallback instanceof ComputedExpression) {
                        ((ComputedExpression)fallback).setLocationId(
    ...
    protected SortKeyDefinition[] makeSortKeys() throws XPathException {
        ...
        if (numberOfSortKeys > 0) {
            ...
            while (true) {
                ...
                if (child instanceof XSLSort) {
                    keys[k++] = ((XSLSort)child).getSortKeyDefinition().simplify(getStaticContext(), getExecutable());
                }
            }
            return keys;
