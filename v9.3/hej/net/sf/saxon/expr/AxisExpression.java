                    ...
                    if (outermostElementNames != null) {
                        ...
                        if (selectedElementNames != null) {
                            if (axis == Axis.CHILD) {
                                ...
                                if (IntHashSet.intersect(selectedElementNames, outermostElementNames).isEmpty()) {
                                ...
                                // check that the name appearing in the step is one of the names allowed by the nodetest
                                boolean canMatchOutermost = !IntHashSet.intersect(selectedElementNames, outermostElementNames).isEmpty();
                ...
                if (contentType.isSimpleType()) {
                    if ((axis == Axis.CHILD || axis==Axis.DESCENDANT || axis==Axis.DESCENDANT_OR_SELF) &&
                        ...
                        env.issueWarning("The " + Axis.axisName[axis] + " axis will never select any " +
                                NodeKindTest.nodeKindName(kind) +
                                " nodes when starting at " +
                                getStartingNodeDescription(contentType), this);
...
//                            }
    ...
    */
    ...
    public final int computeCardinality() {
        ...
        NodeTest contextNodeType;
        if (contextItemType instanceof NodeTest) {
            contextNodeType = (NodeTest)contextItemType;
        } else if (contextItemType instanceof AnyItemType) {
            contextNodeType = AnyNodeTest.getInstance();
        } else {
        ...
        if (axis == Axis.ATTRIBUTE && test instanceof NameTest) {
            SchemaType contentType = contextNodeType.getContentType();
            if (contentType instanceof ComplexType) {
                try {
                    return ((ComplexType)contentType).getAttributeUseCardinality(test.getFingerprint());
            ...
            return StaticProperty.ALLOWS_ZERO_OR_ONE;
        } else if (axis == Axis.DESCENDANT && test instanceof NameTest && test.getPrimitiveType() == Type.ELEMENT) {
            SchemaType contentType = contextNodeType.getContentType();
            if (contentType instanceof ComplexType) {
                try {
                    return ((ComplexType)contentType).getDescendantElementCardinality(test.getFingerprint());
                } catch (SchemaException err) {
                    // shouldn't happen; play safe
                    return StaticProperty.ALLOWS_ZERO_OR_MORE;
                }
