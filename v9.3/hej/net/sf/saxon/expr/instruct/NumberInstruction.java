...
public class NumberInstruction extends Expression {
    ...
    public Expression copy() {
        return new NumberInstruction(
                getContainer().getExecutable().getConfiguration(),
                copy(select), level, count, from, copy(value), copy(format),
                copy(groupSize), copy(groupSeparator), copy(letterValue), copy(ordinal),
    ...
    public Item evaluateItem(XPathContext context) throws XPathException {
            ...
            while (true) {
                ...
                try {
                    ...
                    if (num.compareTo(Int64Value.MAX_LONG) > 0) {
                        vec.add(((BigIntegerValue)num.convert(BuiltInAtomicType.INTEGER, true, rules).asAtomic()).asBigInteger());
                    } else {
                        if (num.compareTo(Int64Value.ZERO) < 0) {
                            throw new XPathException("The numbers to be formatted must not be negative");
                            // thrown to be caught
                        }
                        long i = ((NumericValue)num.convert(BuiltInAtomicType.INTEGER, true, rules).asAtomic()).longValue();
        ...
        if (numb == null) {
            ...
            ConversionResult v = StringValue.convertStringToBuiltInType(
                            language, BuiltInAtomicType.LANGUAGE, rules);
