...
public class SaxonOutputKeys {
    ...
    */
    ...
    private static void checkExtensions(String key, Configuration config) throws XPathException {
        if (config != null && !config.isLicensedFeature(Configuration.LicenseFeature.PROFESSIONAL_EDITION)) {
            throw new XPathException("Serialization parameter " + Err.wrap(key) + " is not available in Saxon-HE", SaxonErrorCode.SXSE0003);
        }
    }
