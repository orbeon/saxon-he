...
public final class TinyElementImpl extends TinyParentNodeImpl {
    ...
    * Get all the inscope namespaces for an element node. This method is better than the generic method
     * provided by {@link net.sf.saxon.tree.iter.NamespaceIterator} because it doesn't require the element node
    ...
    * @param copyOptions
        ...
        do {
			...
			switch (tree.nodeKind[next]) {
				case Type.ELEMENT : {
					...
					// start element
                    final int typeCode = (CopyOptions.includes(copyOptions, CopyOptions.TYPE_ANNOTATIONS) ?
                            tree.getTypeAnnotation(next): StandardNames.XS_UNTYPED);
					...
					if (att >= 0) {
                        while (att < tree.numberOfAttributes && tree.attParent[att] == next ) {
                            ...
                            int attType = (CopyOptions.includes(copyOptions, CopyOptions.TYPE_ANNOTATIONS) ?
     ...
     * @param typeCode  the type annotation of the node
    ...
    private void checkNotNamespaceSensitiveElement(Configuration config, int typeCode, int nodeNr) throws XPathException {
        ...
        if (type instanceof SimpleType && ((SimpleType)type).isNamespaceSensitive()) {
            if (type.isAtomicType()) {
                ...
                Value value = tree.getTypedValueOfElement(nodeNr);
    ...
    private void checkNotNamespaceSensitiveAttribute(Configuration config, int typeCode, int nodeNr) throws XPathException {
        ...
        if (type instanceof SimpleType && ((SimpleType)type).isNamespaceSensitive()) {
            if (type.isAtomicType()) {
                ...
                // For a union or list type, we need to check whether the actual value is namespace-sensitive
                Value value = tree.getTypedValueOfAttribute(null, nodeNr);
                SequenceIterator iter = value.iterate();
                while (true) {
                    AtomicValue val = (AtomicValue)iter.next();
                    if (val == null) {
