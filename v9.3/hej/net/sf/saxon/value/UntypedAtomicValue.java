...
public class UntypedAtomicValue extends StringValue {
    ...
    */
    ...
    private AtomicValue toDouble(ConversionRules rules) throws ValidationException {
        if (doubleValue == null) {
            try {
                double d = rules.getStringToDoubleConverter().stringToNumber(value);
                doubleValue = new DoubleValue(d);
            } catch (NumberFormatException e) {
                throw new ValidationException("Cannot convert string " + Err.wrap(value) + " to a double");
            }
        }
        return doubleValue;
    }
    ...
    public int compareTo(AtomicValue other, StringCollator collator, XPathContext context) {
        if (other instanceof NumericValue) {
            if (doubleValue == null) {
                try {
                    doubleValue = (DoubleValue)convertPrimitive(BuiltInAtomicType.DOUBLE, true,
                            context.getConfiguration().getConversionRules()).asAtomic();
            ...
            ConversionResult result =
                    convert((AtomicType)other.getItemType(th), true, config.getConversionRules());
