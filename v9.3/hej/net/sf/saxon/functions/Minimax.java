    ...
    public static AtomicValue minimax(SequenceIterator iter, int operation,
        ...
        if (foundDouble) {
            if (!(min instanceof DoubleValue)) {
                min = min.convert(BuiltInAtomicType.DOUBLE, true, rules).asAtomic();
            }
        } else if (foundFloat) {
            if (!(min instanceof FloatValue)) {
                min = min.convert(BuiltInAtomicType.FLOAT, true, rules).asAtomic();
            }    
        }
        return min.convert(lowestCommonSuperType, false, rules).asAtomic();
    }
