...
public class ItemTypeFactory {
    ...
    private ItemType getBuiltInAtomicType(int fp) throws SaxonApiException {
        switch (fp) {
            ...
            case StandardNames.XS_PRECISION_DECIMAL:
                return ItemType.PRECISION_DECIMAL;
    ...
    /**
     * Internal method to get a s9api ItemType from a net.sf.saxon.type.ItemType
     */

    ItemType fromInternalItemType(net.sf.saxon.type.ItemType type) throws SaxonApiException {
        if (type.isAtomicType()) {
            AtomicType at = ((AtomicType)type);
            if (at.isBuiltInType()) {
                return getBuiltInAtomicType(at.getFingerprint());
            }
        }
        return new ConstructedItemType(type, processor);
    }
