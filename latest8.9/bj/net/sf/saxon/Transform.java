...
public class Transform {
    ...
    public void doTransform(String args[], String command) {
        ...
        try {
            ...
            if (useAssociatedStylesheet) {
                ...
                if (wholeDirectory) {
                    processDirectory(sources, sheet, outputFile, parameterList, initialTemplate, initialMode);
                } else {
                    Source source = (sources == null ? null : (Source)sources.get(0));
                    processFile(source, sheet, outputFile, parameterList, initialTemplate, initialMode);
                }
            }
    ...
    public void processDirectory(List sources, Templates sheet, File outputDir, ArrayList parameterList,
                                 String initialTemplate, String initialMode)
            throws TransformerException {
        int failures = 0;
        for (int f = 0; f < sources.size(); f++) {
            Source source = (Source)sources.get(f);
    ...
    private void setParams(Transformer t, ArrayList parameterList)
        ...
        for (int i = 0; i < parameterList.size(); i++) {
            ...
            if (argname.startsWith("!")) {
                ...
                t.setOutputProperty(argname.substring(1), argvalue);
    ...
    protected void badUsage(String name, String message) {
        ...
        System.err.println("Usage: " + name + " [options] style-doc {param=value}...");
