...
//    private int nextLocalDocumentNumber = -1;
     ...
     * create the Controller by using the JAXP newTransformer() method.
     *
    ...
    public void setThereHasBeenAnExplicitResultDocument(boolean flag) {
        thereHasBeenAnExplicitResultDocument = flag;
    }
    ...
    private Receiver makeMessageEmitter() throws XPathException {
        ...
        if (!(messageReceiver instanceof Receiver)) {
            throw new DynamicError(emitterClass + " is not a Receiver");
        }
     ...
     * return the CollectionURIResolver previously set using the {@link #setUnparsedTextURIResolver}
    ...
    throws TransformerException {
        ...
        if (initialTemplate == null) {
            ...
            initialContext.setCurrentIterator(single);    // TODO: haven't we already done this once?
            initialContextItem = startNode;
            final Mode mode = getRuleManager().getMode(initialMode, false);
            if (mode == null) {
                throw new DynamicError("Requested initial mode " + getNamePool().getDisplayName(initialMode) +
                        " does not exist", "XTDE0045");
                // TODO: check that we are reporting this error if the mode is named in an applyTemplates
                // instruction but not in any template rules.
            }
     ...
     * @param fingerprint The namepool-allocated fingerprint of the parameter name
     * @param value The value of the parameter
     */

    public void setParameter(int fingerprint, ValueRepresentation value) {
        if (parameters == null) {
            parameters = new GlobalParameterSet();
        }
        parameters.put(fingerprint, value);
    ...
    public Iterator iterateParameters() {
        ...
        int keys[] = parameters.getKeys();
        for (int i=0; i<k; i++) {
            int fp = keys[i];
            String clarkName = namePool.getClarkName(fp);
            list.add(clarkName);
        }
