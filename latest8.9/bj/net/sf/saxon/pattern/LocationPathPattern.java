...
public final class LocationPathPattern extends Pattern {
    ...
    public void addFilter(Expression filter) {
        ...
        Expression.setParentExpression(filter, this);
    }
    ...
    public Pattern analyze(StaticContext env, ItemType contextItemType) throws XPathException {
            ...
            for (int i = numberOfFilters - 1; i >= 0; i--) {
                ...
                if (Literal.isConstantBoolean(filter, true)) {
                    if (i == numberOfFilters - 1) {
                        numberOfFilters--;
                    } // otherwise don't bother doing anything with it. TODO: why not?
                }
            }
        }
        ...
        if (isPositional(th)) {
            ...
            // (see test match55) - removed in 8.9, apparently no longer needed. Caused problems in 8.8 with
            // patterns used for keys in XQuery.
//
...
//
    ...
    public void allocateSlots(ExpressionContext env) {
        int nextFree = 0;
        for (int i = 0; i < numberOfFilters; i++) {
