...
public abstract class Calculator implements Serializable {
    ...
    private static void def(int typeA, int typeB, Calculator[] calculatorSet, String setName) {
        ...
        if (typeA == Type.DURATION) {
            def(Type.DAY_TIME_DURATION, typeB, calculatorSet, setName);
            def(Type.YEAR_MONTH_DURATION, typeB, calculatorSet, setName);
        }
        if (typeB == Type.DURATION) {
            def(typeA, Type.DAY_TIME_DURATION, calculatorSet, setName);
            def(typeA, Type.YEAR_MONTH_DURATION, calculatorSet, setName);
        }
        if (typeA == Type.DATE_TIME) {
            def(Type.DATE, typeB, calculatorSet, setName);
            def(Type.TIME, typeB, calculatorSet, setName);
        }
        if (typeB == Type.DATE_TIME) {
            def(typeA, Type.DATE, calculatorSet, setName);
            def(typeA, Type.TIME, calculatorSet, setName);
        }
        if (typeA == Type.DOUBLE) {
            def(Type.UNTYPED_ATOMIC, typeB, calculatorSet, setName);
        }
        if (typeB == Type.DOUBLE) {
            def(typeA, Type.UNTYPED_ATOMIC, calculatorSet, setName);
        }
    }
    ...
    static {
        def(Type.DOUBLE, Type.DOUBLE, DOUBLE_DOUBLE, "DOUBLE_DOUBLE");
        def(Type.DOUBLE, Type.FLOAT, DOUBLE_FLOAT, "DOUBLE_FLOAT");
        def(Type.DOUBLE, Type.DECIMAL, DOUBLE_DECIMAL, "DOUBLE_DECIMAL");
        def(Type.DOUBLE, Type.INTEGER, DOUBLE_INTEGER, "DOUBLE_INTEGER");
        def(Type.FLOAT, Type.DOUBLE, FLOAT_DOUBLE, "FLOAT_DOUBLE");
        def(Type.FLOAT, Type.FLOAT, FLOAT_FLOAT, "FLOAT_FLOAT");
        def(Type.FLOAT, Type.DECIMAL, FLOAT_DECIMAL, "FLOAT_DECIMAL");
        def(Type.FLOAT, Type.INTEGER, FLOAT_INTEGER, "FLOAT_INTEGER");
        def(Type.DECIMAL, Type.DOUBLE, DECIMAL_DOUBLE, "DECIMAL_DOUBLE");
        def(Type.DECIMAL, Type.FLOAT, DECIMAL_FLOAT, "DECIMAL_FLOAT");
        def(Type.DECIMAL, Type.DECIMAL, DECIMAL_DECIMAL, "DECIMAL_DECIMAL");
        def(Type.DECIMAL, Type.INTEGER, DECIMAL_INTEGER, "DECIMAL_INTEGER");
        def(Type.INTEGER, Type.DOUBLE, INTEGER_DOUBLE, "INTEGER_DOUBLE");
        def(Type.INTEGER, Type.FLOAT, INTEGER_FLOAT, "INTEGER_FLOAT");
        def(Type.INTEGER, Type.DECIMAL, INTEGER_DECIMAL, "INTEGER_DECIMAL");
        def(Type.INTEGER, Type.INTEGER, INTEGER_INTEGER, "INTEGER_INTEGER");
        def(Type.DATE_TIME, Type.DATE_TIME, DATETIME_DATETIME, "DATETIME_DATETIME");
        def(Type.DATE_TIME, Type.DURATION, DATETIME_DURATION, "DATETIME_DURATION");
        def(Type.DURATION, Type.DATE_TIME, DURATION_DATETIME, "DURATION_DATETIME");
        def(Type.DURATION, Type.DURATION, DURATION_DURATION, "DURATION_DURATION");
        def(Type.DURATION, Type.DOUBLE, DURATION_NUMERIC, "DURATION_NUMERIC");
        def(Type.DURATION, Type.FLOAT, DURATION_NUMERIC, "DURATION_NUMERIC");
        def(Type.DURATION, Type.DECIMAL, DURATION_NUMERIC, "DURATION_NUMERIC");
        def(Type.DURATION, Type.INTEGER, DURATION_NUMERIC, "DURATION_NUMERIC");
        def(Type.DOUBLE, Type.DURATION, NUMERIC_DURATION, "NUMERIC_DURATION");
        def(Type.FLOAT, Type.DURATION, NUMERIC_DURATION, "NUMERIC_DURATION");
        def(Type.DECIMAL, Type.DURATION, NUMERIC_DURATION, "NUMERIC_DURATION");
        def(Type.INTEGER, Type.DURATION, NUMERIC_DURATION, "NUMERIC_DURATION");
    }
    ...
    private static class AnyPlusAny extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (calc == null) {
                throw new DynamicError("Unsuitable types for + operation", "XPTY0004", c);
            } else {
    ...
    private static class AnyMinusAny extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (calc == null) {
                throw new DynamicError("Unsuitable types for - operation (" +
    ...
    private static class AnyDivAny extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (calc == null) {
                throw new DynamicError("Unsuitable types for div operation", "XPTY0004", c);
            } else {
                return calc.compute(a, b, c);
            }
        }
    ...
    private static class AnyModAny extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (calc == null) {
                throw new DynamicError("Unsuitable types for mod operation", "XPTY0004", c);
            } else {
                return calc.compute(a, b, c);
            }
        }
    ...
    private static class AnyIdivAny extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (calc == null) {
                throw new DynamicError("Unsuitable types for idiv operation", "XPTY0004", c);
            } else {
                return calc.compute(a, b, c);
            }
        }
    ...
    private static class DoubleIdivDouble extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            if (B == 0.0) {
                throw new DynamicError("Integer division by zero", "FOAR0001", c);
            }
            if (Double.isNaN(A) || Double.isInfinite(A)) {
                throw new DynamicError("First operand of idiv is NaN or infinity", "FOAR0002", c);
            }
            if (Double.isNaN(B)) {
                throw new DynamicError("Second operand of idiv is NaN", "FOAR0002", c);
            }
            return new DoubleValue(A / B).convert(BuiltInAtomicType.INTEGER, c);
        }
        public AtomicType getResultType(AtomicType typeA, AtomicType typeB) {
    ...
    private static class FloatIdivFloat extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            return new FloatValue(A / B).convert(BuiltInAtomicType.INTEGER, c);
        }
        public AtomicType getResultType(AtomicType typeA, AtomicType typeB) {
            return BuiltInAtomicType.INTEGER;
        }
        ...
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            try {
                ...
                if (((NumericValue)b).compareTo(0) == 0) {
                    throw new DynamicError("Decimal divide by zero", "FOAR0001", c);
                } else {
                    throw err;
                }
            }
    ...
    private static class DecimalModDecimal extends Calculator {
        public AtomicValue compute(AtomicValue a, AtomicValue b, XPathContext c) throws XPathException {
            ...
            try {
                ...
                if (((NumericValue)b).compareTo(0) == 0) {
                    throw new DynamicError("Decimal modulo zero", "FOAR0001", c);
                } else {
                    throw err;
                }
            }
