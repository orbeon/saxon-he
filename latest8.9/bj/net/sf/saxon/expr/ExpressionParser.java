...
public class ExpressionParser {
    ...
    // The following switch is used to suppress compile-time evaluation of expression, by wrapping
    // a LazyExpression around string and numeric literals, and certain other constructs that compile
    // to a Value. This is provided for testing purposes only (it allows greater coverage of run-time
    // code paths).

    private final static boolean SUPPRESS_EARLY_EVALUATION = false;

    public ExpressionParser(){
        if (SUPPRESS_EARLY_EVALUATION) {
            System.err.println("*** ExpressionParser: switch set for lazy evaluation of literals ***");
        }
    }
    ...
    protected void warning(String message) throws StaticError {
        ...
        // TODO: different implementations of issueWarning differ in whether they add prefix "Warning: "
    }
     ...
     * @throws net.sf.saxon.trans.StaticError if the QName is invalid or if no atomic type of that
...
//            return exp;
    ...
    private Expression parseBasicStep() throws StaticError {
        ...
        case Token.LPAR:
            ...
            if (t.currentToken==Token.RPAR) {
                nextToken();
                if (SUPPRESS_EARLY_EVALUATION) {
                    return new LazyExpression(new Literal(EmptySequence.getInstance()));
                }
        ...
        case Token.STRING_LITERAL:
            ...
            nextToken();
            if (SUPPRESS_EARLY_EVALUATION) {
                return new LazyExpression(literal);
            }
            return literal;
        ...
        case Token.NUMBER:
            ...
            nextToken();
            if (SUPPRESS_EARLY_EVALUATION) {
                return new LazyExpression(new Literal(number));
            }
    ...
    protected Literal makeStringLiteral(String currentTokenValue) throws StaticError {
...
//                        }
    ...
    protected Expression parseFunctionCall() throws StaticError {
        ...
        if (SUPPRESS_EARLY_EVALUATION && fcall instanceof Literal) {
            return new LazyExpression(fcall);
        }
     ...
     *
     * @throws net.sf.saxon.trans.StaticError if the name is invalid, or the prefix
     *     undeclared
    ...
    protected void setLocation(Expression exp, int offset) {
        ...
        if (exp.getLocationId()==-1) {
            ...
            if (exp.getParentExpression() == null) {
                ...
                exp.setParentExpression(container);
            }
        }
    }
