...
public class UserFunctionCall extends FunctionCall implements InstructionInfoProvider {
    ...
    */
    ...
    public boolean isRecursiveTailCall() {
        if (tailCall) {
            Expression p = this;
            while (true) {
                Container c = p.getParentExpression();
                if (c instanceof UserFunction) {
                    return (c == function);
                }
    ...
    public Expression typeCheck(StaticContext env, ItemType contextItemType) throws XPathException {
        ...
        if (function != null) {
            ...
            if (staticType == SequenceType.ANY_SEQUENCE) {
                ...
                staticType = function.getResultType(env.getConfiguration().getTypeHierarchy());
            }
        }
        return e;
    }
    ...
    */
    ...
    public int markTailFunctionCalls(int nameCode, int arity) {
        tailCall = true;
        return (((nameCode & NamePool.FP_MASK) == (getFunctionNameCode() & NamePool.FP_MASK) &&
