...
//        }
    ...
    public static boolean isRepeatedSubexpression(Expression parent, Expression child, StaticContext env) {
        ...
        if (parent instanceof FilterExpression) {
            ...
            // removed below code because it was causing an optimization loop with let $x := a+1 return $z[$x]
//                    &&
//                    !th.isSubType(child.getItemType(th), BuiltInAtomicType.NUMERIC);
        }
        ...
        if (parent instanceof ForEachGroup) {
            return child == ((ForEachGroup)parent).getActionExpression() ||
                    child == ((ForEachGroup)parent).getGroupingKey();
        }
...
//        }
    ...
    throws XPathException {
        ...
        if (exp2 != exp) {
            Expression.setParentExpression(exp2, exp.getParentExpression());
            return exp2;
        }
        return exp;
    }
    ...
    public static int markTailFunctionCalls(Expression exp, int nameCode, int arity) {
        return exp.markTailFunctionCalls(nameCode, arity);
    }
    ...
    */
    ...
    private static int allocateSlots(Expression exp, int nextFree, SlotManager frame, boolean topLevel) {
        ...
        if (exp instanceof VariableReference && ((VariableReference)exp).getBinding() instanceof Assignation &&
                ((VariableReference)exp).getBinding().getLocalSlotNumber() < 0) {
            ...
            Assignation decl = (Assignation)var.getBinding();
            System.err.println("*** Internal Saxon error: Unbound local variable encountered");
            ...
            decl.display(10, System.err, decl.getExecutable().getConfiguration());
            Container c = decl.getParentExpression();
            if (c instanceof Expression) {
                System.err.println("PARENT OF DECLARATION:");
                ((Expression)c).display(10, System.err, decl.getExecutable().getConfiguration());
            }
            throw new IllegalStateException("*** Internal Saxon error: Unbound local variable encountered");
