...
public class XSLStylesheet extends StyleElement {
    ...
    public void setPreparedStylesheet(PreparedStylesheet sheet) {
        ...
        if (config.isAllowExternalFunctions()) {
            ...
            Configuration.getPlatform().addFunctionLibraries(functionLibrary, config);
        }
    ...
    protected StringCollator findCollation(String name) {
        ...
        if (exec.getCollationTable() != null) {
            c = (StringCollator) exec.getCollationTable().getNamedCollation(name);
        }
        if (c != null) return c;
    ...
    private void indexVariableDeclaration(XSLVariableDeclaration var) throws XPathException {
        ...
        if (fingerprint != -1) {
            ...
            XSLVariableDeclaration other = (XSLVariableDeclaration) globalVariableIndex.get(fingerprint);
            if (other == null) {
                // this is the first
                globalVariableIndex.put(fingerprint, var);
            } else {
    ...
    public XSLVariableDeclaration getGlobalVariable(int fingerprint) {
        return (XSLVariableDeclaration) globalVariableIndex.get(fingerprint);
    }
