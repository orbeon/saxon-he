...
public abstract class NumericValue extends AtomicValue implements Comparable {
    ...
    public static NumericValue parseNumber(String in) {
        if (in.indexOf('e') >= 0 || in.indexOf('E') >= 0) {
            ...
            AtomicValue v = Int64Value.stringToInteger(in);
            if (v instanceof ValidationErrorValue) {
                return DoubleValue.NaN;
            } else {
                return (NumericValue)v;
    ...
    public double getDoubleValue() {
        try {
            return ((DoubleValue)convert(BuiltInAtomicType.DOUBLE, null)).getDoubleValue();
    ...
    public float getFloatValue() {
        try {
            return ((FloatValue)convert(BuiltInAtomicType.FLOAT, null)).getFloatValue();
        } catch (XPathException err) {
            return Float.NaN;
        }
    }
    ...
    public BigDecimal getDecimalValue() throws XPathException {
        return ((DecimalValue)convert(BuiltInAtomicType.DECIMAL, null)).getDecimalValue();
    }
    ...
    public long longValue() throws XPathException {
        return ((Int64Value)convert(BuiltInAtomicType.INTEGER, null)).longValue();
    }
     ...
     *     {@link net.sf.saxon.expr.Token#PLUS}, {@link net.sf.saxon.expr.Token#MINUS}
    ...
    // TODO: still used, but should be replaced
