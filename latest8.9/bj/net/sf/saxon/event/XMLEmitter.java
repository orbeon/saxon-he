    ...
    throws java.io.IOException, XPathException {
        ...
        while (segstart < chars.length()) {
            ...
            if (c==0) {
                ...
                if (c > 127) {
                    if (XMLChar.isHighSurrogate(c)) {
                        int cc = XMLChar.supplemental(c, chars.charAt(i+1));
                        if (!characterSet.inCharset(cc)) {
                            DynamicError de = new DynamicError("Character " + cc + " (" + (int)cc + ") is not available in the chosen encoding");
                            de.setErrorCode("SERE0008");
                            throw de;
                        }
                    } else if (!characterSet.inCharset(c)) {
                        DynamicError de = new DynamicError("Character " + c + " (" + (int)c + ") is not available in the chosen encoding");
                        de.setErrorCode("SERE0008");
                        throw de;
                    }
                }
