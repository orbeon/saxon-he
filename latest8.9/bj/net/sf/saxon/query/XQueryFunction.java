...
public class XQueryFunction implements InstructionInfo, Container, Declaration {
    ...
    public UserFunction compile(StaticQueryContext env) throws StaticError {
        ...
        try {
            ...
            if (compiledFunction == null) {
                ...
                body = body.simplify(staticContext).typeCheck(staticContext, null);
                body = body.optimize(staticContext.getConfiguration().getOptimizer(), staticContext, null);
                Expression.setParentExpression(body, this);
                RoleLocator role =
                ...
                int tailCalls = ExpressionTool.markTailFunctionCalls(body, nameCode, params.length);
                if (tailCalls != 0) {
                ...
                for (int i=0; i<params.length; i++) {
                    ...
                    int refs = decl.getReferenceCount(param, staticContext);
    ...
    public void explain(Configuration config, PrintStream out) {
        ...
        out.println("declare function " + pool.getDisplayName(nameCode) +
                (compiledFunction.isTailRecursive() ? "(:tail recursive:) {" : " {"));
        body.display(4, System.err, config);
        out.println("}");
    }
