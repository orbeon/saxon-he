...
public class ForEachGroup extends Instruction implements ContextMappingFunction, SortKeyEvaluator {
        ...
        if (sortKeys != null) {
            ...
            for (int i=0; i<sortKeys.length; i++) {
                ...
                if (sortKeys[i].isFixed()) {
                    AtomicComparer comp = sortKeys[i].makeComparator(env.makeEarlyEvaluationContext());
        ...
        if (collator == null && (collationNameExpression instanceof StringLiteral)) {
            ...
            try {
                ...
                if (!collationURI.isAbsolute()) {
                    ...
                    collator = env.getCollation(collationNameString);
                    if (collator == null) {
                ...
                StaticError e = new StaticError("Collation name '" + collationNameExpression + "' is not a valid URI");
    ...
    public void display(int level, PrintStream out, Configuration config) {
        ...
        out.println(ExpressionTool.indent(level) + getAlgorithmName(algorithm));
        key.display(level + 1, out, config);
