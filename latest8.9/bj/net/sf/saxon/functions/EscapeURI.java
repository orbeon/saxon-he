    ...
    */
    ...
    public static void checkPercentEncoding(String uri) throws XPathException {
        for (int i=0; i<uri.length();) {
            ...
            int expectedOctets = 0;
            if (c == '%') {
                if (i+2 >= uri.length()) {
                    throw new DynamicError("% sign in URI must be followed by two hex digits" +
                                    Err.wrap(uri));
                }
                ...
                if (h1 >= 0 && h2 >= 0) {
                    ...
                    if (expectedOctets == -1) {
                        throw new DynamicError("First %-encoded octet in URI is not valid as the start of a UTF-8 " +
                    ...
                    for (int q=1; q<expectedOctets; q++) {
                        if (i+2 > uri.length() || uri.charAt(i) != '%') {
                            throw new DynamicError("Incomplete %-encoded UTF-8 octet sequence in URI " +
                                    Err.wrap(uri));
                        }
                        ...
                        if (h1 < 0 || h2 < 0) {
                            throw new DynamicError("Invalid %-encoded UTF-8 octet sequence in URI" +
                                    Err.wrap(uri));
                        }
                        if (UTF8RepresentationLength[h1] != -1) {
                            throw new DynamicError("In a URI, a %-encoded UTF-8 octet after the first " +
