    ...
    private ItemType convertDotNetToXPathType(cli.System.Type resultClass) {
        ...
        if (name.equals("net.sf.saxon.value.Value")) {
            ...
            return BuiltInAtomicType.FLOAT;
                ...
                name.equals("System.Byte") ||name.equals("net.sf.saxon.value.IntegerValue")) {
        ...
        if (targetType == CLI_STRING) {
            ...
            try {
                ...
                if (v instanceof DoubleValue) {
                    ...
                    return new Decimal((long)((Int64Value)v).longValue());
    ...
    private static AtomicValue getAtomicValue(Value value, cli.System.Type targetType, XPathContext context) throws XPathException {
        if (value instanceof UntypedAtomicValue) {
            ...
            if (targetType == CLI_BOOLEAN) {
                value = uav.convertPrimitive(BuiltInAtomicType.BOOLEAN, true, context);
                ...
                value = uav.convertPrimitive(BuiltInAtomicType.INTEGER, true, context);
            } else if (targetType == CLI_DOUBLE) {
                value = uav.convertPrimitive(BuiltInAtomicType.DOUBLE, true, context);
            } else if (targetType == CLI_SINGLE) {
                value = uav.convertPrimitive(BuiltInAtomicType.FLOAT, true, context);
            } else if (targetType == CLI_DECIMAL) {
                value = uav.convertPrimitive(BuiltInAtomicType.DECIMAL, true, context);
            } else {
