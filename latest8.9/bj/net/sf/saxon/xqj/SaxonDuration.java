...
public class SaxonDuration extends Duration {
    ...
    public void addTo(Calendar calendar) {
        ...
        try {
            ...
            calendar.add((int)((Int64Value)duration.getComponent(Component.WHOLE_SECONDS)).longValue()*sign, Calendar.SECOND);
            calendar.add((int)((Int64Value)duration.getComponent(Component.MICROSECONDS)).longValue()*sign/1000, Calendar.MILLISECOND);
        } catch (XPathException e) {
            throw new IllegalStateException(e.getMessage());
        }
    }
    ...
    public int compare(Duration rhs) {
        ...
        if (duration instanceof YearMonthDurationValue) {
            return ((YearMonthDurationValue)duration).compareTo((YearMonthDurationValue)((SaxonDuration)rhs).duration);
        } else if (duration instanceof DayTimeDurationValue) {
            return ((DayTimeDurationValue)duration).compareTo((DayTimeDurationValue)((SaxonDuration)rhs).duration);
        } else {
