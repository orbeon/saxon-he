...
public abstract class SaxonXQDataFactory implements XQDataFactory {
    ...
    static {
        ...
        map(XQItemType.XQBASETYPE_XDT_ANYATOMICTYPE, StandardNames.XS_ANY_ATOMIC_TYPE);
        map(XQItemType.XQBASETYPE_XDT_DAYTIMEDURATION, StandardNames.XS_DAY_TIME_DURATION);
        map(XQItemType.XQBASETYPE_XDT_UNTYPED, StandardNames.XS_UNTYPED);
        map(XQItemType.XQBASETYPE_XDT_UNTYPEDATOMIC, StandardNames.XS_UNTYPED_ATOMIC);
        map(XQItemType.XQBASETYPE_XDT_YEARMONTHDURATION, StandardNames.XS_YEAR_MONTH_DURATION);
    }
    ...
    public XQItem createItemFromByte(byte value, XQItemType type) throws XQException {
        if (type == null) {
            try {
                return new SaxonXQItem(new Int64Value(value, BuiltInAtomicType.BYTE, false),
                        getConfiguration());
    ...
    public XQItem createItemFromInt(int value, XQItemType type) throws XQException {
        if (type == null) {
            try {
                return new SaxonXQItem(
                        new Int64Value(value, BuiltInAtomicType.INT, false), getConfiguration());
            } catch (DynamicError de) {
                throw new XQException(de.getMessage(), de, null, null);
            }
        } else {
    ...
    public XQItem createItemFromLong(long value, XQItemType type) throws XQException {
        if (type == null) {
            try {
                return new SaxonXQItem(
                        new Int64Value(value, BuiltInAtomicType.LONG, false), getConfiguration());
            } catch (DynamicError de) {
                throw new XQException(de.getMessage(), de, null, null);
            }
        } else {
            ...
            if (prim == Type.INTEGER || prim == Type.DECIMAL) {
                try {
                    AtomicValue av = Int64Value.makeIntegerValue(value).convert(at, null, true);
                    return new SaxonXQItem(av, getConfiguration());
                } catch (Exception e) {
    ...
    private XQItem convertToXQItem(Object value, XQItemType type) throws XQException {
        if (value instanceof Boolean) {
            ...
            int prim = at.getPrimitiveType();
            if (prim == Type.QNAME) {
                QualifiedNameValue dv = (QualifiedNameValue) getCommonHandler().fromObject(value);
                return new SaxonXQItem(dv.convert(at, null, true), getConfiguration());
            } else {
    ...
    public XQItem createItemFromShort(short value, XQItemType type) throws XQException {
        if (type == null) {
            try {
                return new SaxonXQItem(
                        new Int64Value(value, BuiltInAtomicType.SHORT, false), getConfiguration());
            } catch (DynamicError de) {
                throw new XQException(de.getMessage(), de, null, null);
            }
        } else {
