...
public class ExpressionTool {
    ...
    */
    ...
    private static int allocateSlots(Expression exp, int nextFree, SlotManager frame, boolean topLevel) {
        ...
        if (topLevel) {
            // TODO: this process is probably now redundant, as LocalVariableReferences are allocated from the start
            for (Iterator children = exp.iterateSubExpressions(); children.hasNext();) {
                Expression child = (Expression)children.next();
                refineVariableReference(exp, child);
            }
        }
        return nextFree;
    ...
    private static void refineVariableReference(Expression parent, Expression child) {
        if (child instanceof VariableReference) {
            ((VariableReference)child).refineVariableReference(parent);
        } else {
            for (Iterator grandChildren = child.iterateSubExpressions(); grandChildren.hasNext();) {
                Expression grandChild = (Expression)grandChildren.next();
                refineVariableReference(child, grandChild);
            }
        }
    }
