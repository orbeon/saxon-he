...
public final class FilterExpression extends Expression {
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ItemType contextItemType) throws XPathException {
        ...
        filter2 = ExpressionTool.unsortedIfHomogeneous(visitor.getConfiguration().getOptimizer(), filter, false);
        if (filter2 != filter) {
            filter = filter2;
            adoptChildExpression(filter2);
        }
...
//        }
    ...
    public Expression optimize(ExpressionVisitor visitor, ItemType contextItemType) throws XPathException {
        ...
        // TODO: If this is still a filter expression, then the only thing the filter can depend on is the focus.
        // So if we're filtering a constant value, we can do it now

        // This doesn't work because we can't spawn a new XPathMinorContext from an EarlyEvaluationContext
...
//        return result;
    ...
    public Expression promote(PromotionOffer offer) throws XPathException {
        ...
        if (exp != null) {
            ...
            if (offer.action == PromotionOffer.RANGE_INDEPENDENT && start instanceof FilterExpression) {
                ...
                FilterExpression newfe = promoteIndependentPredicates(offer.bindingList, th);
                if (newfe != this) {
    ...
    private FilterExpression promoteIndependentPredicates(Binding[] bindings, TypeHierarchy th) {
        ...
        if (start instanceof FilterExpression) {
            ...
            if (!ExpressionTool.dependsOnVariable(filter, bindings)) {
                return new FilterExpression(
                        new FilterExpression(fe.start, filter).promoteIndependentPredicates(bindings, th),
                        fe.filter);
            }
        }
        return this;
