...
class QueryParser extends ExpressionParser {
    ...
    /**
     * Protected Constructor: this class should be instantiated via the QueryModule
     */
...
//        ep.close();
    ...
    private void applySchemaImport(Import sImport) throws XPathException {
        ...
        ((QueryModule)env).addImportedSchema(sImport.namespaceURI);
    }
...
//        }
    ...
    public void applyModuleImport(Import mImport) throws XPathException {
        ...
        for (int m = 0; m < sources.length; m++) {
            ...
            // TODO: use similar logic when loading schema modules or stylesheet modules
    ...
    private void checkProhibitedPrefixes(String prefix, String uri) throws XPathException {
        ...
        if (uri.equals(NamespaceConstant.XMLNS)) {
            grumble("The xmlns namespace URI is reserved");
        }
        if (uri.equals(NamespaceConstant.XML) && !prefix.equals("xml")) {
            grumble("The XML namespace cannot be bound to any prefix other than 'xml'");
        }
        if (prefix.equals("xml") && !uri.equals(NamespaceConstant.XML)) {
            grumble("The prefix 'xml' cannot be bound to any namespace other than " + NamespaceConstant.XML);
        }
    }
        ...
        switch (t.currentToken) {
            ...
            case Token.KEYWORD_CURLY:
                ...
                if (nodeKind.equals("validate")) {
                    ...
                    doc.setValidationAction(((QueryModule)env).getConstructionMode());
...
//    }
    ...
    private Expression parseDirectElementConstructor() throws XPathException, StringIndexOutOfBoundsException {
        ...
        while (true) {
            ...
            // TODO: see bug 5083. We are anticipating a change such that the value of a namespace
            // URI is not required to be a valid DirAttributeValue
...
//                }
                String prefix;
                // Handle doubled quotation marks within the URI
                if (rval.indexOf(delim) >= 0) {
                    String doubleDelim = "" + delim + delim;
                    while (true) {
                        int d = rval.indexOf(doubleDelim);
                        if (d < 0) {
                            break;
                        }
