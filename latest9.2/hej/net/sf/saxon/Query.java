...
public class Query {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].startsWith("-config:")) {
                ...
                try {
                    ...
                    outputProperties = new Properties(config.getDefaultSerializationProperties());
                    schemaAware = config.isLicensedFeature(Configuration.LicenseFeature.SCHEMA_AWARE_XQUERY);
    ...
    protected void parseOptions(String[] args, String command, StaticQueryContext staticEnv,
        ...
        while (i < args.length) {
            ...
            if (args[i].charAt(0) == '-') {
                ...
                if (option.startsWith("-")) {
                    // --feature:value
                    try {
                        String feature = "http://saxon.sf.net/feature/" + option.substring(1);
                        config.setConfigurationProperty(feature, value);
                        i++;
                    } catch (Exception err) {
                        badUsage(command, "unrecognized feature " + option + ": " + err.getMessage());
                    }
...
//    }
    ...
    protected void badUsage(String name, String message) {
        ...
        System.err.println("  -sa                   Schema-aware query (requires Saxon-EE)");
        ...
        System.err.println("  -update:on|off|discard  Enable|Disable XQuery Update (needs Saxon-EE)");
