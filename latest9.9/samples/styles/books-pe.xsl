...
<!-- A variant of books.xsl that uses reflexive extension functions, and therefore needs Saxon-PE.  -->
...
<xsl:template match="/">
    ...
    <xsl:comment>SAXON_HOME = <xsl:value-of select="System:getenv('SAXON_HOME')" xmlns:System="java:java.lang.System"/></xsl:comment>
    <xsl:copy-of select="unparsed-text('books.txt')"/>
