 ...
 * <p>This is the implementation of the NodeInfo interface used as a wrapper for JDOM nodes.</p>
 *
...
public class JDOMNodeWrapper extends AbstractNodeWrapper implements SiblingCountingNode {
    ...
    /**
     * Get the underlying JDOM node, to implement the VirtualNode interface. If this wrapper
    ...
    private static class DescendantWrappingIterator<B> extends NodeWrappingAxisIterator<B> {

        public DescendantWrappingIterator(Iterator<B> descendantIterator, NodeWrappingFunction<B, NodeInfo> wrappingFunction) {
    ...
    public NamespaceBinding[] getDeclaredNamespaces(NamespaceBinding[] buffer) {
        if (node instanceof Element) {
            ...
            // Add a binding for the element name itself, unless the binding is xmlns="", which we add only if it
            // differs from the parent element
            NamespaceBinding extra = null;
            Namespace ns = elem.getNamespace();
            String prefix = ns.getPrefix();
            String uri = ns.getURI();
            if (!prefix.isEmpty() || !uri.isEmpty()) {
                extra = new NamespaceBinding(prefix, uri);
            }
            int size = addl.size() + (extra == null ? 0 : 1);
            ...
            if (forwards) {
                if (children.hasNext()) {
                    ...
                    if (nextChild instanceof EntityRef) {
                        ...
                        JDOMNodeWrapper current = makeWrapper(nextChild, getTreeInfo(), commonParent, ix++);
