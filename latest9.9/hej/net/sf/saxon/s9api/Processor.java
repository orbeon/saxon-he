...
public class Processor implements Configuration.ApiProvider {
        ...
        try {
            if (destination instanceof Serializer) {
                ...
                for (XdmItem item : value) {
                    tree.append(item.getUnderlyingValue(), ExplicitLocation.UNKNOWN_LOCATION, ReceiverOptions.ALL_NAMESPACES);
                }
                tree.endDocument();
                tree.close();
    ...
    private SchemaManager makeSchemaManager() {
        ...
        //#if EE==true
        manager = new com.saxonica.ee.s9api.SchemaManagerImpl(this);
        //#endif
        return manager;
    }
