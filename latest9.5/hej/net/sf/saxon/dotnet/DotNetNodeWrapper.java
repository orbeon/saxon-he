...
public class DotNetNodeWrapper extends AbstractNodeWrapper implements SteppingNode, SiblingCountingNode {
     ...
     */
    protected DotNetNodeWrapper makeWrapper(XmlNode node, DotNetDocumentWrapper docWrapper) {
        if (node == null) {
    ...
    public int getNameCode() {
        ...
        if (nodeKind == Type.ELEMENT || nodeKind == Type.ATTRIBUTE) {
            ...
            namecode = getNamePool().allocate(prefix, getURI(), getLocalPart());
            return namecode;
    ...
    protected AxisIterator<NodeInfo> iterateChildren(NodeTest nodeTest) {
        AxisIterator<NodeInfo> iter = new ChildEnumeration(this, true, true);
        if (nodeTest != AnyNodeTest.getInstance()) {
            iter = new Navigator.AxisFilter(iter, nodeTest);
        }
        return iter;
    ...
    protected AxisIterator<NodeInfo> iterateSiblings(NodeTest nodeTest, boolean forwards) {
        AxisIterator<NodeInfo> iter = new ChildEnumeration(this, false, forwards);
        if (nodeTest != AnyNodeTest.getInstance()) {
            iter = new Navigator.AxisFilter(iter, nodeTest);
        }
        return iter;
    ...
    private final class AttributeEnumeration implements AxisIterator<NodeInfo>, LookaheadIterator<NodeInfo> {
        ...
        public NodeInfo next() {
            ...
            current = start.makeWrapper(attList.get(ix), docWrapper, start, ix);
            ix++;
            return current;
        }
