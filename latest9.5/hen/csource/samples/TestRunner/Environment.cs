...
public class
    ...
    public Dictionary<string, FileStream> streamedSecondaryDocs = new Dictionary<string, FileStream>();
    ...
    public static Environment createLocalEnvironment(Uri baseURI, int generateByteCode, bool unfolded) {
        ...
        foreach (XdmItem source in xpc.Evaluate("source", env)) {
            ...
            if ("true".Equals(streaming)) {
                if (".".Equals(role)) {
                    ...
                    try
                    {
                        environment.streamedSecondaryDocs.Add(validation, new FileStream(res.ResolveUri(((XdmNode)env).BaseUri, href).AbsolutePath, FileMode.Open, FileAccess.Read));
                    }catch(Exception){}
                }
            } else {
                Stream ss;
                if (uri != null) {
                    uri = res.ResolveUri(((XdmNode)env).BaseUri, uri).AbsolutePath;
                }
                if (href != null) {
