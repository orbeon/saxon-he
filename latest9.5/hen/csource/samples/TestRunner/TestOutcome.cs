...
//                            ", got " + actual);
    ...
    private bool AssertSerializationMatches(XdmNode assertion, SingleResultDoc result,XPathCompiler catalogXpc) {
        if (IsException()) {
            ...
            try {
                java.lang.CharSequence stringBuffer = regex;
                JRegularExpression re = JConfiguration.getPlatform().compileRegularExpression(stringBuffer, flagsAtt, "XP30", new java.util.ArrayList());
    ...
    public bool compareExpectedError(XdmNode assertion) {
        ...
        bool ok = (expectedError.Equals("*") ||
                (GetErrorCode() != null &&
                        GetErrorCode().Equals(expectedErrorQ)) ||
