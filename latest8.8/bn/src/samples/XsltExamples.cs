...
public class XsltExamples {
    ...
    public static void Main(String[] argv) {
        ...
        if (test == "all" || test == "ExampleUsingOutputURIResolver") {
            Console.WriteLine("\n\n==== ExampleUsingOutputURIResolver ====");

            try {
                ExampleUsingOutputURIResolver(foo_xml, multidoc_xsl);
            } catch (Exception ex) {
                handleException(ex);
            }
        }
    ...
    */
    ...
    public static void ExampleUsingOutputURIResolver(String sourceUri, String xsltUri) {
    ...
    public static void ExtraTestCase(String sourceURI, String xsltUri) {
        Processor processor = new Processor();
        XsltCompiler compiler = processor.NewXsltCompiler();
        Stream xsltStream = new FileStream("c:/MyDotNet/samples/trax/xsl/foo.xsl", FileMode.Open, FileAccess.Read);
        XmlReader reader = new XmlTextReader(xsltStream);
        compiler.BaseUri = new Uri("file:///c:/MyDotNet/samples/trax/xsl/");
        //compiler.ErrorList = new ArrayList();
