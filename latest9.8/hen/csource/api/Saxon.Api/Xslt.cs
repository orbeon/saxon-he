...
using JRoleDiagnostic = net.sf.saxon.expr.parser.RoleDiagnostic;
using JStylesheetPackage = net.sf.saxon.style.StylesheetPackage;
using JXsltPackage = net.sf.saxon.s9api.XsltPackage;
using JPackageInspector = net.sf.saxon.trans.packages.PackageInspector;
...
namespace Saxon.Api
{
    ...
    public class XsltCompiler
    {
        ...
        /// </summary>
        /// <remarks>true if bytecode is to be generated, false if not</remarks>
    ...
    public class XsltExecutable
    {
        ...
        public JPreparedStylesheet Implementation
        {
            get
            {
                return pss;
        ...
        /// and whose values are <see cref="ParameterDetails"/> objects giving information about the
    ...
    public class XsltTransformer : XdmDestination
    {
        ...
        public XdmNode InitialContextNode
        {
            ...
            set
            {
                if (value == null)
                {
                    initialContextNode = null;
                    controller.setGlobalContextItem(null);
                }
                else
                {
                    initialContextNode = (JNodeInfo)value.Unwrap();
        ...
        public RecoveryPolicy RecoveryPolicy
        {
            get
            {
                ...
                {
                    case JConfiguration.RECOVER_SILENTLY:
                        return RecoveryPolicy.RecoverSilently;
                    case JConfiguration.RECOVER_WITH_WARNINGS:
            ...
            set
            {
                controller.setRecoveryPolicy(
                    value == RecoveryPolicy.RecoverSilently ? JConfiguration.RECOVER_SILENTLY :
                    value == RecoveryPolicy.RecoverWithWarnings ? JConfiguration.RECOVER_WITH_WARNINGS :
                    JConfiguration.DO_NOT_RECOVER);
            }
        }
    ...
    internal class MessageListenerProxy : JSequenceWriter
    {
        ...
        public override void characters(CharSequence s, JLocation location, int properties)
        {
            ...
            base.characters(s, location, properties);
        }
    ...
    public class Xslt30Transformer
    {
        ...
        public XdmValue CallFunction(QName function, XdmValue[] arguments)
        {
            ...
            try
            {
                net.sf.saxon.trans.SymbolicName fName = new net.sf.saxon.trans.SymbolicName.F(function.ToStructuredQName(), arguments.Length);
                ...
                JUserFunction uf = (JUserFunction)f.getActor();
    ...
    public class XsltPackage
    {
        ...
        public XsltExecutable Link()
        {
            ...
            {
                JConfiguration config = processor.Implementation;
                JCompilation compilation = new JCompilation(config, config.makeCompilerInfo());
