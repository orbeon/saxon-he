        ...
        switch (n1.getNodeKind()) {
            case Type.ELEMENT:
                if (!Navigator.haveSameName(n1, n2)) {
                    explain(config, "element names differ: " + NameOfNode.makeName(n1).getStructuredQName().getClarkName() +
                            " != " + NameOfNode.makeName(n2).getStructuredQName().getClarkName(), flags, n1, n2);
                    return false;
                }
                ...
                while ((att1 = a1.next()) != null) {
                    ...
                    if (att2 == null) {
                        explain(config, "one element has an attribute " +
                            NameOfNode.makeName(att1).getStructuredQName().getClarkName() +
                    ...
                    if (!deepEquals(att1, att2, comparer, config, flags)) {
                        ...
                        explain(config, "elements have different values for the attribute " +
                            NameOfNode.makeName(att1).getStructuredQName().getClarkName(), flags, n1, n2);
                        return false;
                    }
                }
                ...
                if ((flags & COMPARE_STRING_VALUES) == 0) {
                    ...
                    if (isSimple1 && isSimple2) {
                        ...
                        return deepEqual(v1, v2, comparer, config.getConversionContext(), flags);
                    }
                }
            ...
            case Type.ATTRIBUTE:
                if (!Navigator.haveSameName(n1, n2)) {
                    explain(config, "attribute names differ: " +
                        NameOfNode.makeName(n1).getStructuredQName().getClarkName() +
                            " != " + NameOfNode.makeName(n1).getStructuredQName().getClarkName(), flags, n1, n2);
                    return false;
                }
                if (((flags & INCLUDE_PREFIXES) != 0) && !n1.getPrefix().equals(n2.getPrefix())) {
                ...
                boolean ar;
                if ((flags & COMPARE_STRING_VALUES) == 0) {
                    ar = deepEqual(n1.atomize().iterate(), n2.atomize().iterate(), comparer, config.getConversionContext(), 0);
                } else {
