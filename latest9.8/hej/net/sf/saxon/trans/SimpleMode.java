...
public class SimpleMode extends Mode {
    ...
    protected RuleChain genericRuleChain = new RuleChain();
    ...
    public void addRule(Pattern pattern, Rule newRule) {
        ...
        if (uType.equals(UType.ELEMENT)) {
            ...
            if (fp == -1) {
                addRuleToList(newRule, unnamedElementRuleChain);
            } else {
                RuleChain chain = namedElementRuleChains.get(fp);
                if (chain == null) {
                    chain = new RuleChain(newRule);
                    namedElementRuleChains.put(fp, chain);
                } else {
            ...
            int fp = pattern.getFingerprint();
            if (fp == -1) {
                addRuleToList(newRule, unnamedAttributeRuleChain);
            } else {
                RuleChain chain = namedAttributeRuleChains.get(fp);
                if (chain == null) {
                    chain = new RuleChain(newRule);
                    namedAttributeRuleChains.put(fp, chain);
                } else {
                    addRuleToList(newRule, chain);
                }
            }
            ...
            if (pattern instanceof NodeTestPattern || pattern instanceof BasePatternWithPredicate) {
