...
public abstract class Pattern extends PseudoExpression {
    ...
    public static Pattern make(String pattern, StaticContext env, PackageData packageData) throws XPathException {
        ...
        PatternParser parser = (PatternParser) env.getConfiguration().newExpressionParser("PATTERN", false, 30);
        ...
        if (UType.DOCUMENT.subsumes(uType)) {
            ...
            MappingFunction atts = new MappingFunction() {
                public SequenceIterator map(Item item) {
                    return ((NodeInfo)item).iterateAxis(AxisInfo.ATTRIBUTE);
                }
            };
            ...
            MappingFunction atts = new MappingFunction() {
                public SequenceIterator map(Item item) {
                    return ((NodeInfo)item).iterateAxis(AxisInfo.NAMESPACE);
                }
            };
