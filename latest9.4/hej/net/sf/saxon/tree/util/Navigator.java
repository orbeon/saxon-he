...
public final class Navigator {
    ...
    /*@Nullable*/ public static NodeInfo getOutermostElement(/*@NotNull*/ DocumentInfo doc) {
        return doc.iterateAxis(Axis.CHILD, NodeKindTest.ELEMENT).next();
    }
    ...
    public static int getNumberSingle(/*@NotNull*/ NodeInfo node, /*@Nullable*/ Pattern count,
        ...
        SequenceIterator preceding =
                target.iterateAxis(Axis.PRECEDING_SIBLING, (NodeTest)count.getItemType());
            ...
            case Type.ELEMENT:
                {
                    SchemaType annotation = ((copyOptions & CopyOptions.TYPE_ANNOTATIONS) != 0 ?
                            node.getSchemaType() :
                            Untyped.getInstance());
                    out.startElement(new NameOfNode(node), annotation, 0, 0);
            ...
            case Type.ATTRIBUTE:
                {
                    ...
                    out.attribute(new NameOfNode(node), annotation, node.getStringValueCS(), 0, 0);
                    return;
                }
            case Type.TEXT:
                {
                ...
                if (start.hasChildNodes()) {
                    ...
                    if (!forwards) {
                        if (children instanceof ReversibleIterator) {
                            ...
                            AxisIterator forwards = start.iterateAxis(Axis.CHILD);
                            while (true) {
