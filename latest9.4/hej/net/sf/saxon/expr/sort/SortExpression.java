...
public class SortExpression extends Expression
    ...
    private Iterator<Expression> iterateSubExpressions(boolean includeSortKey) {
        ...
        for (int i = 0; i < sortKeyDefinitions.length; i++) {
            if (includeSortKey || sortKeyDefinitions[i].isSetContextForSortKey()) {
        ...
        if (sortKeyDefinitions != null) {
            for (int i = 0; i < sortKeyDefinitions.length; i++) {
                if (sortKeyDefinitions[i].isSetContextForSortKey()) {
                    ...
                    sortKeyDefinitions[i].getSortKey().addToPathMap(pathMap, pathMapNodeSet);
                }
    ...
    public boolean replaceSubExpression(Expression original, Expression replacement) {
        ...
        for (int i = 0; i < sortKeyDefinitions.length; i++) {
            if (sortKeyDefinitions[i].getSortKey() == original) {
                sortKeyDefinitions[i].setSortKey(replacement, true);
                found = true;
            }
    ...
    public Expression optimize(ExpressionVisitor visitor, ExpressionVisitor.ContextItemType contextItemType) throws XPathException {
        ...
        for (int i = 0; i < sortKeyDefinitions.length; i++) {
            ...
            sortKeyDefinitions[i].setSortKey(sortKey, true);
        }
    ...
    public Expression promote(PromotionOffer offer, Expression parent) throws XPathException {
        ...
        if (exp != null) {
            ...
            for (int i = 0; i < sortKeyDefinitions.length; i++) {
                ...
                sortKeyDefinitions[i].setSortKey(sk2, true);
