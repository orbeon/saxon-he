...
public class XdmValue implements Iterable<XdmItem> {
    ...
    private ValueRepresentation<? extends Item> value;    // this must be a materialized value
    ...
    /*@Nullable*/ protected static XdmValue wrap(ValueRepresentation<? extends Item> value) {
        if (value == null) {
            return null;
    ...
    public XdmItem itemAt(int n) throws IndexOutOfBoundsException, SaxonApiUncheckedException {
        ...
        if (value instanceof Value) {
            try {
                Item item = ((Value) value).itemAt(n);
                return (XdmItem)XdmItem.wrap(item);
            } catch (XPathException e) {
                throw new SaxonApiUncheckedException(e);
            }
        } else {
            return (XdmNode)XdmNode.wrap(value);
        }
    }
