...
public class QueryModule implements StaticContext {
    ...
    private void init(/*@Nullable*/ StaticQueryContext sqc) {
        ...
        variables = new HashMap<StructuredQName, VariableDeclaration>(10);
        undeclaredVariables = new HashMap<StructuredQName, UndeclaredVariable>(5);
        if (isTopLevelModule()) {
            libraryVariables = new HashMap<StructuredQName, GlobalVariableDefinition>(10);
        }
...
//        }
    ...
    public void declareVariable(/*@NotNull*/ VariableDeclaration var) throws XPathException {
        ...
        final HashMap<StructuredQName, GlobalVariableDefinition> libVars = getTopLevelModule().libraryVariables;
        GlobalVariableDefinition old = libVars.get(key);
        if (old == null || old == var) {
            // do nothing
        } else {
            XPathException err = new XPathException("Duplicate definition of global variable "
    ...
    /*@NotNull*/ public List<GlobalVariableDefinition> fixupGlobalVariables(/*@NotNull*/ SlotManager globalVariableMap) throws XPathException {
    ...
    public void lookForModuleCycles(/*@NotNull*/ Stack<QueryModule> referees, int lineNumber) throws XPathException {
        if (referees.contains(this)) {
            ...
            while (viter.hasNext()) {
                ...
                Expression select = (gvc == null ? null : gvc.getSelectExpression());
                if (select != null) {
    ...
    public void typeCheckGlobalVariables(/*@NotNull*/ List compiledVars) throws XPathException {
        ...
        for (Object compiledVar : compiledVars) {
            GlobalVariableDefinition gvd = (GlobalVariableDefinition) compiledVar;
    ...
    /*@NotNull*/ public Expression bindVariable(/*@NotNull*/ StructuredQName qName) throws XPathException {
        ...
        if (var == null) {
            ...
            if (var instanceof GlobalVariableDefinition &&
                    ((GlobalVariableDefinition)var).isPrivate() &&
                    !((GlobalVariableDefinition)var).getSystemId().equals(getSystemId())) {
               XPathException err = new XPathException("Variable $" + qName.getDisplayName() + " is private");
                err.setErrorCode("XPST0008");
                err.setIsStaticError(true);
                throw err;
    ...
    public void bindUnboundVariables() throws XPathException {
        for (UndeclaredVariable uv : undeclaredVariables.values()) {
            ...
            if (var == null) {
                ...
                err.setIsStaticError(true);
                throw err;
            } else if (var instanceof GlobalVariableDefinition &&
                    ((GlobalVariableDefinition) var).isPrivate() &&
                    !((GlobalVariableDefinition) var).getSystemId().equals(getSystemId())) {
...
//         }
    ...
    /*@NotNull*/ public NamespaceResolver getNamespaceResolver() {
        ...
        List<NamespaceBinding> nsBindings = new ArrayList();
        ...
        if (externalNamespaceCodes != null) {
            for (int a = 0; a < externalNamespaceCodes.size(); a++) {
                nsBindings.add(externalNamespaceCodes.get(a));
            }
        }
