...
//    private static int instances = 0;
    ...
    /*@NotNull*/
    public static <T extends Item> ValueRepresentation<T> makeSequenceExtent(/*@NotNull*/ SequenceIterator<T> iter) throws XPathException {
        if ((iter.getProperties() & SequenceIterator.GROUNDED) != 0) {
            return ((GroundedIterator<T>)iter).materialize();
        }
        Value<T> extent = new SequenceExtent<T>(iter);
        int len = extent.getLength();
        if (len==0) {
            return EmptySequence.getInstance();
        } else if (len==1) {
            T item = extent.itemAt(0);
            assert item != null;
            return item;
        } else {
            return extent;
    ...
    public static <T extends Item> ValueRepresentation makeSequenceExtent(/*@NotNull*/ List<T> input) {
        int len = input.size();
        if (len==0) {
            return EmptySequence.getInstance();
        } else if (len==1) {
    ...
    public ItemType getItemType(/*@Nullable*/ TypeHierarchy th) {
        ...
        if (it != null) {
            ...
            for (int i=start+1; i<end; i++) {
                ...
                it = Type.getCommonSuperType(it, Type.getItemType(value[i], null));
            }
        } else {
