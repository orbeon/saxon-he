...
//				exp = trace;
     ...
     * @return if compilation detects any static errors
     * @throws net.sf.saxon.trans.XPathException if compilation fails
	 */
	...
	throws XPathException {
		...
		while (true) {
			...
			if (node.getNodeKind() == Type.TEXT) {
				...
				LocalVariable lv = ((XSLVariable)node).compileLocalVariable(exec, decl);
				if (lv == null) {
	...
	throws XPathException {
		...
		while (true) {
			...
			if (child instanceof XSLFallback) {
				...
				Expression b = ((XSLFallback)child).compileSequenceConstructor(exec, decl, child.iterateAxis(Axis.CHILD), true);
				if (b == null) {
