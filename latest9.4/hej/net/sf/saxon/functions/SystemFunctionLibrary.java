...
public class SystemFunctionLibrary implements FunctionLibrary {
    ...
    /*@Nullable*/ public SequenceType[] getFunctionSignature(StructuredQName functionName, int arity) {
        ...
        if (uri.equals(NamespaceConstant.FN)) {
            ...
            if (entry == null) {
                if ((local.equals("true") || local.equals("false")) && arity < 1) {
                    // these functions are special-cased
    ...
    public Expression bind(StructuredQName functionName, Expression[] staticArgs, StaticContext env, Container container)
        ...
        if (uri.equals(NamespaceConstant.FN)) {
            String local = functionName.getLocalPart();
            if (staticArgs.length == 0 && (local.equals("true") || local.equals("false"))) {
                Literal lit = new Literal(BooleanValue.get(local.equals("true")));
                lit.setContainer(container);
                return lit;
            }
