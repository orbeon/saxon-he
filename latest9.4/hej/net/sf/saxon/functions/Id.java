...
public class Id extends SystemFunction implements CallableExpression {
    ...
    * property bit is set, it is true, but if it is unset, the value is unknown.
    ...
    public PathMap.PathMapNodeSet addToPathMap(PathMap pathMap, PathMap.PathMapNodeSet pathMapNodeSet) {
        ...
        target = target.createArc(Axis.DESCENDANT, NodeKindTest.ELEMENT);
        return target;
    }
    ...
    */
    ...
    public SequenceIterator call(SequenceIterator[] arguments, XPathContext context) throws XPathException {
        NodeInfo arg1;
        try {
            arg1 = (NodeInfo)arguments[1].next();
        } catch (XPathException e) {
            if (context.getContextItem() instanceof AtomicValue) {
                // Override the unhelpful message that trickles down...
                XPathException e2 = new XPathException("id() function called when the context item is not a node");
        ...
        if (isSingletonId) {
            AtomicValue arg = (AtomicValue)arguments[0].next();
            if (arg==null) {
                return EmptyIterator.getInstance();
            }
            String idrefs = arg.getStringValue();
