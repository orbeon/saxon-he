...
public final class LocationPathPattern extends Pattern {
    ...
    /*@Nullable*/ private Pattern upperPattern = null;
    ...
    public Pattern simplify(ExpressionVisitor visitor) throws XPathException {
        ...
        if (upperPattern == null &&
                ...
                !firstElementPattern &&
                !lastElementPattern) {
            ItemTypePattern ntp = new ItemTypePattern(nodeTest);
    ...
    public Pattern analyze(ExpressionVisitor visitor, ExpressionVisitor.ContextItemType contextItemType) throws XPathException {
        ...
        for (int i = filters.length - 1; i >= 0; i--) {
            Expression filter = visitor.typeCheck(filters[i], cit);
            filter = ExpressionTool.unsortedIfHomogeneous(opt, filter);
            filter = visitor.optimize(filter, cit);
            filters[i] = filter;
