...
std::cerr<<"phpNativeCall YYYY ----"<<std::endl;
php_error(E_WARNING, "phpNativeCall");
	JNIEnv *senv = SaxonProcessor::sxn_environ->env;

	const char *nativeString = senv->GetStringUTFChars(funcName, NULL);

	if(nativeString == NULL) {
	php_error(E_WARNING, "nativeString is null!!");	
	}
...
std::cerr<<"--- CHECKPOINT 0-1 --- nativeString ="<<nativeString<<" argLength="<<argLength<<std::endl;
...
std::cerr<<"--- CHECKPOINT 0 --- nativeString ="<<nativeString<<std::endl;
	for(int i=0; i<argLength;i++){
std::cerr<<"phpNativeCall checkpoint 1 ----"<<std::endl;
		jstring argType = (jstring)env->GetObjectArrayElement(argTypes, i);
		jobject argObj = env->GetObjectArrayElement(arguments, i);
		...
		if (it != typeMap.end()){
			switch (it->second)
			{
				case enumNode:
					...
					tempObj = php_argv[i]._val;
					MAKE_STD_ZVAL(tempObj);
					object_init_ex(tempObj, xdmNode_ce);
   					vobj = (struct xdmNode_object *)zend_object_store_get_object(tempObj TSRMLS_CC);
                			assert (vobj != NULL);
                			vobj->xdmNode = node;
					break;
				case enumString:
				...
				case enumInteger:
					...
					dnumber = getLongValue(*SaxonProcessor::sxn_environ, *sresult, 0);
					MAKE_STD_ZVAL(php_argv[i]._val);
					ZVAL_LONG(php_argv[i]._val, lnumber);					
					break;
				case enumDouble:
					sresult->xdmvalue = argObj; 
					lnumber = getDoubleValue(*SaxonProcessor::sxn_environ, *sresult, 0);
					MAKE_STD_ZVAL(php_argv[i]._val);
...
std::cerr<<"--- CHECKPOINT 4 ---"<<std::endl;
                ...
                break;
            case IS_STRING:
		std::cerr<<"--- STRING TYPE RETURNING ---"<<std::endl;
