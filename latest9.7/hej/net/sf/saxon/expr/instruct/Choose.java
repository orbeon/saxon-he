...
public class Choose extends Instruction {
        ...
        for (int i = 0; i < size(); i++) {
            ...
            try {
                ...
                if (err.isTypeError()) {
                    ...
                    setAction(i, new ErrorExpression(err));
                }
            }
        }
        return this;
    ...
    private Expression removeRedundantBranches0() {
        ...
        int size = size();
        if (compress) {
            List<Expression> conditions = new ArrayList<Expression>(size);
        ...
        if (size == 2 &&
                ...
                Literal.isConstantBoolean(getCondition(1), true)) {
    ...
    public Expression typeCheck(ExpressionVisitor visitor, ContextItemStaticInfo contextInfo) throws XPathException {
        for (int i = 0; i < size(); i++) {
            ...
            XPathException err = TypeChecker.ebvError(getCondition(i), getConfiguration().getTypeHierarchy());
            if (err != null) {
        ...
        for (int i = 0; i < size(); i++) {
            try {
                ...
                if (err.isStaticError()) {
                    ...
                    if (Literal.isEmptySequence(getAction(i)) || Literal.isConstantBoolean(getCondition(i), false)) {
                        setAction(i, new ErrorExpression(err));
                    } else {
                        throw err;
                    }
    ...
    public Expression staticTypeCheck(SequenceType req,
        ...
        for (int i = 0; i < size; i++) {
            try {
                setAction(i, TypeChecker.staticTypeCheck(getAction(i), req, backwardsCompatible, role, visitor));
            } catch (XPathException err) {
                if (err.isStaticError()) {
                    throw err;
                } else if (err.isTypeError()) {
                ...
                ExpressionTool.copyLocationInfo(getAction(0), ee);
                setAction(i, ee);
            }
        }
    ...
    public Expression optimize(ExpressionVisitor visitor, ContextItemStaticInfo contextItemType) throws XPathException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (getCondition(i) instanceof Literal &&
                ...
                setCondition(i, Literal.makeLiteral(BooleanValue.get(b)));
            }
        }
        for (int i = 0; i < size; i++) {
            try {
    ...
    public int computeCardinality() {
        ...
        for (int i = 0; i < size(); i++) {
            ...
            if (Literal.isConstantBoolean(getCondition(i), true)) {
                includesTrue = true;
            }
        }
        if (!includesTrue) {
    ...
    protected void promoteChildren(PromotionOffer offer) throws XPathException {
        ...
        if (offer.action == PromotionOffer.EXTRACT_GLOBAL_VARIABLES) {
            for (int i = 0; i < size(); i++) {
                setCondition(i, doPromotion(getCondition(i), offer));
            }
            for (int i = 0; i < size(); i++) {
                setAction(i, doPromotion(getAction(i), offer));
            }
        } else {
            // in other cases, only the first xsl:when condition is promoted
            setCondition(0, doPromotion(getCondition(0), offer));
        }
    }
    ...
    public EventIterator iterateEvents(XPathContext context) throws XPathException {
        int i = choose(context);
        return i < 0 ? EmptyEventIterator.getInstance() : getAction(i).iterateEvents(context);
    }
