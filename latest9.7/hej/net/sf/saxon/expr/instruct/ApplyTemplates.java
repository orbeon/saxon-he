...
public class ApplyTemplates extends Instruction implements ITemplateCall, ComponentInvocation {
    ...
    protected TailCall apply(XPathContext context, boolean returnTailCall) throws XPathException {
        ...
        Component<? extends Mode> targetMode = getTargetMode(context);
        ...
        if (returnTailCall) {
            ...
            return new ApplyTemplatesPackage(
                    ExpressionTool.lazyEvaluate(getSelect(), context, 1),
                    targetMode, params, tunnels, c2, getLocation());
        }
    ...
    public Component<? extends Mode> getTargetMode(XPathContext context) {
        Component<? extends Mode> targetMode;
        if (useCurrentMode) {
            targetMode = context.getCurrentMode();
        } else {
            if (bindingSlot >= 0) {
                targetMode = (Component<? extends Mode>)context.getTargetComponent(bindingSlot);
    ...
    protected static class ApplyTemplatesPackage implements TailCall {
        ...
        public TailCall processLeavingTail() throws XPathException {
            evaluationContext.setCurrentIterator(new FocusTrackingIterator(selectedItems.iterate()));
            evaluationContext.setCurrentMode(targetMode);
            evaluationContext.setCurrentComponent(targetMode);
            return targetMode.getCode().applyTemplates(params, tunnelParams, evaluationContext, locationId);
        }
    }
