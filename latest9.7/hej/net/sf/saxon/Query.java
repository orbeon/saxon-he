...
public class Query {
    ...
    /*@Nullable*/ protected String languageVersion = null;
    ...
    public void setPermittedOptions(CommandLineOptions options) {
        ...
        options.addRecognizedOption("qversion", CommandLineOptions.TYPE_ENUMERATION | CommandLineOptions.VALUE_REQUIRED,
                "Indicate which XQuery version is supported");
        options.setPermittedValues("qversion", new String[]{"1.0", "3.0", "3.1"}, null);
...
//            DecimalValue decimalVerson = new DecimalValue(new BigDecimal(languageVersion));
//            if (decimalVerson.equals(DecimalValue.THREE) && !config.isLicensedFeature(Configuration.LicenseFeature.PROFESSIONAL_EDITION)) {
...
//            }
    ...
    protected void parseOptions(CommandLineOptions options)
        ...
        try {
            ...
            languageVersion = qv;
            DecimalValue dv = (DecimalValue) DecimalValue.makeDecimalValue(qv, true).asAtomic();
...
//        return null;
            ...
            while (true) {
                ...
                try {
                    ...
                    if (e instanceof XPathException) {
                        ...
                        SaxonApiException err = new SaxonApiException(xe);
                        throw err;
                    }
                }
                if (sources == null) {
    ...
    protected void runQuery(XQueryExecutable exp, XQueryEvaluator evaluator, Source input, Destination destination)
        ...
        if (wrap && !pullMode) {
            try {
                ...
                XdmValue wrappedResultDoc = XdmValue.wrap(resultDoc.getUnderlyingNode());
            ...
            try {
                ...
                if (!SequenceReceiver.isTrueSequenceReceiver(r)) {
                    r = new TreeReceiver(r);
                }
                e.pull(evaluator.getUnderlyingQueryContext(), (SequenceReceiver) r);
            } catch (XPathException e1) {
                throw new SaxonApiException(e1);
            }
        } else if (streaming) {
        ...
        if (closeTraceDestination) {
            evaluator.getTraceFunctionDestination().close();
        }
    }
    ...
    protected void runUpdate(XQueryExecutable exp, XQueryEvaluator evaluator, final Serializer serializer)
        ...
        if (writeback) {
            final List<SaxonApiException> errors = new ArrayList<SaxonApiException>(3);
    ...
    public static void rewriteToDisk(NodeInfo doc, Serializer serializer, boolean backup, PrintStream log)
            throws SaxonApiException {
    ...
    protected void badUsage(String message) {
        ...
        System.err.println("Usage: see http://www.saxonica.com/html/documentation/using-xquery/commandline.html");
