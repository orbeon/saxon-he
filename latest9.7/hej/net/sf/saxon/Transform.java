...
public class Transform {
    ...
    public void setPermittedOptions(CommandLineOptions options) {
       ...
       /* options.addRecognizedOption("if", CommandLineOptions.TYPE_QNAME | CommandLineOptions.VALUE_REQUIRED,
        ...
        options.addRecognizedOption("pack", CommandLineOptions.TYPE_FILENAME | CommandLineOptions.VALUE_REQUIRED,
        ...
        options.addRecognizedOption("xsl", CommandLineOptions.TYPE_FILENAME_LIST | CommandLineOptions.VALUE_REQUIRED,
        ...
        options.setPermittedValues("xsltversion", new String[]{"0.0", "2.0", "3.0"}, null);
    ...
    public void doTransform(String args[], String command) {
        ...
        //boolean diag = false;           // project:preconditions
        //String diagOutputFileName = null;  // project:preconditions
        //RuleTraceListener ruleTraceListener = null;
        ...
        try {
            ...
            value = options.getOptionValue("diag");
            if (value != null) {
                if (!"".equals(value)) {
                    traceListener = new RuleTraceListener();
                    if (value.length() > 0) {
                        String v1 = value;
                        traceDestination = new StandardLogger(new File(v1));
            ...
            value = options.getOptionValue("pack");
            if (value != null) {
                packageCatalogName = value;
            }
...
//            }
            ...
            if (useAssociatedStylesheet) {
                ...
                for (String sn : styleFileNames) {
                    ...
                    stylesheetSources.add(styleSource);
                }

                // Initialise a package library for the compiler.
                // We do it here so that we can incrementally add.
                // There will be problems however if repeat > 1
                compiler.newPackageLibrary();

                if (packageCatalogName != null) {
                    Source packSource;
                    if (useURLs || CommandLineOptions.isImplicitURI(packageCatalogName)) {
                        packSource = config.getURIResolver().resolve(packageCatalogName, null);
                        if (packSource == null) {
                            packSource = config.getSystemURIResolver().resolve(packageCatalogName, null);
                        }
                    } else {
                        File catalogFile = new File(packageCatalogName);
                        if (!catalogFile.exists()) {
                            quit("Package catalog file " + catalogFile + " does not exist", 2);
                        }
                        packSource = new StreamSource(catalogFile.toURI().toString());
                    }
                    try {
                        PackageCatalog catalog = new PackageCatalog(processor, packSource);
                        compiler.addCompilePackages(catalog.getSources(), true);
                    } catch (SaxonApiException e) {
                        e.printStackTrace();
                        quit(e.getMessage(), 2);
                    }
                }
                PackageLibrary pl = compiler.getPackageLibrary();

                if (stylesheetSources.size() > 1) {
                    for (int j = 0; j < repeat; j++) {
                        // Repeat loop is to get reliable performance data
                        if (repeat > 1) {
                            compiler.setPackageLibrary(pl.copy()); // we need to re-initialise the library for repeats
                        }
                        List<XsltPackage> xsltPackages = (List<XsltPackage>) compiler.addCompilePackages(stylesheetSources, false);
                        for (XsltPackage packagei : xsltPackages) {
                            //compiler.importPackage(packagei);
                            StylesheetPackage pp = packagei.getUnderlyingPreparedPackage();
                            if (pp.isRootPackage()) {
                                sheet = packagei.link();
                            }
                        }
                        if (showTime) {
                            long endTime = now();
                            System.err.println("Stylesheet compilation time: "
                                + CommandLineOptions.showExecutionTimeNano(endTime - startTime));
                            startTime = endTime;
                        }
                    }
                } else {
                    styleSource = stylesheetSources.get(0);
                    for (int j = 0; j < repeat; j++) {
                        // Repeat loop is to get reliable performance data
                        if (repeat > 1) {
                            compiler.setPackageLibrary(pl.copy()); // we need to re-initialise the library for repeats
                        ...
                        ///////////////////////////////////////
                        if (schemaAware) {
                            int licenseId = ((StylesheetPackage)sheet.getUnderlyingCompiledStylesheet().
    ...
    protected void processFile(/*@Nullable*/ Source source, XsltExecutable sheet, /*@Nullable*/ File outputFile, CommandLineOptions options,
        ...
        //RealPreconditions.totalTime(totalTime);    // project:preconditions
        if (repeat > 1) {
