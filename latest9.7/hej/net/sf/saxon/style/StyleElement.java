...
public abstract class StyleElement extends ElementImpl {
        ...
        if (container == null || !(container instanceof StylesheetComponent)) {
            throw new XPathException(
                    "A reference to xsl:original appears within the wrong kind of component: in this case" +
                            ", it must be within " + getNamePool().getDisplayName(componentKind), "XTSE0650", this);
        }
    ...
    public void check30attribute(String attName) throws XPathException {
        if (!isXslt30Processor()) {
            ...
            if (forwardsCompatibleModeIsEnabled()) {
                compileWarning("The @" + attName + " attribute of " + getDisplayName() +
                                       " is ignored in forwards-compatibility mode", "");
            } else {
                compileErrorInAttribute("The @" + attName + " attribute of " + getDisplayName() +
                                                " is not recognized by an XSLT 2.0 processor", "XTSE0090", attName);
            }
        }
    }
    ...
    public boolean processBooleanAttribute(String name, String value) throws XPathException {
        ...
        if (isXslt30Processor() || forwardsCompatibleModeIsEnabled()) {
            // Not a strict interpretation of the forwards compatibility rules for 2.0, but it's more helpful
    ...
    public SequenceType makeSequenceType(String sequenceType)
        ...
        try {
            XPathParser parser =
                    getConfiguration().newExpressionParser("XP", false, staticContext.getXPathVersion());
            parser.setLanguage(XPathParser.XPATH, staticContext.getXPathVersion());
    ...
    public boolean forwardsCompatibleModeIsEnabled() {
        return getEffectiveVersion() > getProcessorVersion();
    }
     ...
     *
     * @return the version of XSLT supported by the processor, times ten as an integer
     */

    public int getProcessorVersion() {
        return getCompilation().getProcessorVersion();
    }
    ...
    public Expression typeCheck(String name, Expression exp) throws XPathException {
        ...
        try {
            exp = exp.typeCheck(makeExpressionVisitor(), new ContextItemStaticInfo(Type.ITEM_TYPE, true));
    ...
    public Expression compileSequenceConstructor(Compilation compilation, ComponentDeclaration decl,
        ...
        while ((node = (NodeInfo) iter.next()) != null) {
            if (node.getNodeKind() == Type.TEXT) {
                if (isExpandingText()) {
                    getCompilation().getStyleNodeFactory(true).compileContentValueTemplate((TextImpl) node, contents);
                } else {
        ...
        if (containsSpecials) {
            return getCompilation().getStyleNodeFactory(false).makeConditionalBlock(contents);
        }
    ...
    protected WithParam[] getWithParamInstructions(Expression parent, Compilation compilation, ComponentDeclaration decl, boolean tunnel)
            throws XPathException {
        int count = 0;
        AxisIterator kids = iterateAxis(AxisInfo.CHILD);
        NodeInfo child;
