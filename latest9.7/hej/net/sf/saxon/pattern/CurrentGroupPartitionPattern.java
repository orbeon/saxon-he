 ...
 */
public class CurrentGroupPartitionPattern extends Pattern {

    private GroupBoundaryTester groupBoundaryTester;
    private NodeTest nodeTest = AnyNodeTest.getInstance();


    public CurrentGroupPartitionPattern(GroupBoundaryTester groupBoundaryTester) {
        this.groupBoundaryTester = groupBoundaryTester;
    };
    ...
    public boolean matchesBeneathAnchor(NodeInfo node, NodeInfo anchor, XPathContext context) throws XPathException {
        boolean matches = anchor == null || node == anchor;
        if (matches) {
            maybeStartNewGroup(node, context);
        }
        return matches;
    }
    ...
    public boolean matches(Item item, XPathContext context) throws XPathException {
        maybeStartNewGroup(item, context);
        return true;
    }

    private void maybeStartNewGroup(Item item, XPathContext context) throws XPathException {
        boolean startNew = groupBoundaryTester.notifyItem();
        ManualGroupIterator gi = (ManualGroupIterator) context.getCurrentGroupIterator();
        gi.setContextItem(context.getContextItem());
        if (startNew) {
            gi.setPosition(1);
        } else {
            gi.incrementPosition();
        }
    }
