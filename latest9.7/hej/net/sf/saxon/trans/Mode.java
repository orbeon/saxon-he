...
public abstract class Mode extends ComponentCode<SimpleMode> {
     ...
     * @return a new XPath context
     */
    public XPathContext makeNewContextIfNeeded(XPathContext context) {
        if (getStackFrameSlotsNeeded() > 0) {
    ...
    public Rule getNextMatchRule(Item item, final Rule currentRule, XPathContext context) throws XPathException {
        SimpleMode.RuleFilter filter = new SimpleMode.RuleFilter() {
            public boolean testRule(Rule r) {
    ...
    public TailCall applyTemplates(
        ...
        while (true) {
            ...
            if (tracing && traceListener instanceof TraceListener2) {
                ((TraceListener2) traceListener).endRuleSearch((rule != null) ? rule : getBuiltInRuleSet(), this, item);
            }
    ...
    /**
     * Return a built-in rule set for a code string. This can be specialised in subclasses for PE/EE
     *
