...
public class MemoClosure extends Closure {
    ...
    /*@Nullable*/ private List<Item> reservoir = null;
    protected int state;
        ...
        switch (state) {
            case UNREAD:
                ...
                if (in instanceof EmptyIterator) {
                    state = EMPTY;
                    return inputIterator = EmptyIterator.emptyIterator();
                } else if ((in.getProperties() & SequenceIterator.GROUNDED) != 0) {
                    GroundedValue value = ((GroundedIterator)in).materialize();
                    reservoir = new ArrayList<Item>(value.getLength());
                    Item it;
                    while ((it = in.next()) != null) {
                        reservoir.add(it);
                    }
                    state = ALL_READ;
                    ...
                    inputIterator = in;
                    reservoir = new ArrayList<Item>(50);
                    state = MAYBE_MORE;
            ...
            case ALL_READ:
                switch (reservoir.size()) {
                    case 0:
                        state = EMPTY;
                        return EmptyIterator.emptyIterator();
                    case 1:
                        return SingletonIterator.makeIterator(reservoir.get(0));
                    default:
                        return new ListIterator(reservoir);
                }
    ...
    public synchronized void process(/*@NotNull*/ XPathContext context) throws XPathException {
        ...
        if (reservoir != null) {
            ...
            while ((it = iter.next()) != null) {
                out.append(it, ExplicitLocation.UNKNOWN_LOCATION, NodeInfo.ALL_NAMESPACES);
            }
        } else {
            state = BUSY;
    ...
    private void condense() {
        if (reservoir != null) {
            ((ArrayList) reservoir).trimToSize();
        }
        // give unwanted stuff to the garbage collector
        savedXPathContext = null;
//        inputIterator = null;
//        expression = null;
    ...
    public synchronized Item itemAt(int n) throws XPathException {
        ...
        if (reservoir != null && n < reservoir.size()) {
            return reservoir.get(n);
        }
    ...
    public final class ProgressiveIterator implements SequenceIterator, LastPositionFinder, GroundedIterator {
        ...
        public Item next() throws XPathException {
            synchronized (MemoClosure.this) {
                // synchronized "just in case" the same variable is being referenced and evaluated by code in
                // multiple threads. The code in LetExpression attempts to ensure that eager evaluation is used
                // in such cases (because otherwise, the lock acquired here can potentially be held for a long time),
                // but we synchronize anyway for safety.
                if (position == -2) {   // means we've already returned null once, keep doing so if called again.
                    return null;
                }
                if (++position < reservoir.size()) {
                    return reservoir.get(position);
